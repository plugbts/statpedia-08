...
async function analyzeGameLogsOnly() {
  ...
  try {
    ...
    Object.entries(playersBySeason).forEach(([season, players]) => {
      console.log(`  ${season}: ${players.size} unique players, ${recordsBySeason[season]} records`);
    });
    ...
    topPlayers.forEach(([playerId, stats], i) => {
      console.log(`  ${i + 1}. ${stats.name} (${playerId}): ${stats.records} records, ${stats.propTypes.size} prop types`);
    });
    ...
    Object.entries(propTypeStats)
      ...
      .forEach(([propType, stats]) => {
        console.log(`  ${propType}: ${stats.records} records, ${stats.players.size} players, avg: ${stats.avgValue.toFixed(1)}`);
      });
    ...
    const jaxonData = gameLogs?.filter(log => log.player_name.includes('JAXON SMITHNJIGBA')).slice(0, 10);
