...
async function testLeagueAwareFix() {
  ...
  try {
    ...
    const testMappings = [
      { league: 'nfl', expectedProps: ['passing_yards', 'rushing_yards', 'rushing_attempts', 'receptions', 'passing_touchdowns'] },
    ...
    const totalOverUnder = Object.values(leagueStats).reduce((sum, stats) => sum + stats.overUnder, 0);
    ...
    if (totalOverUnder === 0) {
      ...
      console.log(`⚠️  PARTIAL: ${totalOverUnder} over/under props still remain across all leagues`);
