-- Check for existing functions and resolve conflicts
-- This script will drop existing functions and recreate them with proper signatures

-- Step 1: Drop existing functions to avoid conflicts
DROP FUNCTION IF EXISTS get_api_usage_stats();
DROP FUNCTION IF EXISTS get_api_usage_stats(UUID);
DROP FUNCTION IF EXISTS get_api_usage_stats(UUID, TIMESTAMP WITH TIME ZONE);
DROP FUNCTION IF EXISTS get_api_usage_stats(UUID, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE);

DROP FUNCTION IF EXISTS get_api_usage_vs_plan();
DROP FUNCTION IF EXISTS get_api_usage_vs_plan(UUID);

DROP FUNCTION IF EXISTS log_api_usage(UUID, TEXT, TEXT, TEXT, INTEGER, INTEGER, BOOLEAN, TEXT, TEXT, INET);

-- Step 2: Recreate functions with explicit signatures
...
-- Step 3: Create get_api_usage_stats with explicit parameter types
...
-- Step 4: Create get_api_usage_vs_plan with explicit parameter types
...
-- Step 6: Test the functions with explicit calls
SELECT 'Testing API usage functions with explicit parameters...' as test_step;
