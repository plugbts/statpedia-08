...
async function testOptimizedQueryStrategy() {
  ...
  try {
    ...
    const { data: optimizedResults, error: optimizedError } = await supabase
      .rpc('get_player_props_with_game_logs', {
        p_league: 'nfl',
        p_season: 2025,
    ...
    if (sampleProplines && sampleGameLogs) {
      for (const prop of sampleProplines) {
        for (const gameLog of sampleGameLogs) {
          ...
          if (gameLog.player_id === prop.player_id &&
            ...
            if (normalizedPropType === normalizedPropPropType) {
              ...
              if (dayDiff <= 1) {
                successfulMatches++;
                console.log(`✅ Match found: ${gameLog.player_name} - ${gameLog.prop_type} (${dayDiff.toFixed(2)} days diff)`);
              }
            }
          }
        }
    ...
    console.log(`🎯 Match rate: ${totalAttempts > 0 ? (successfulMatches / totalAttempts * 100).toFixed(2) : 0}%`);
