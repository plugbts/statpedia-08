...
__export(fetchProps_exports, {
  ...
  ingestAndEnrich: () => ingestAndEnrich,
...
async function fetchRawProps(env, league, dateISO) {
  ...
  try {
    ...
    console.log(`\u{1F50D} Using league config: ${leagueConfig.id} ${leagueConfig.seasons[0]} with oddIDs: ${leagueConfig.oddIDs}`);
    ...
    if (dateISO && events.length > 0) {
      ...
      console.log(`\u{1F50D} Filtered ${filteredEvents.length} events for date ${dateISO} from ${events.length} total events`);
...
async function ingestAndEnrich(env, league, dateISO) {
  console.log(`\u{1F680} Starting dual-mode ingestion for ${league} on ${dateISO}`);
...
async function persistProps(env, enriched) {
  if (!enriched.length) {
...
async function buildProps(env, league, dateISO) {
  console.log(`\u{1F680} Starting dual-mode props build for ${league} on ${dateISO}`);
  const enriched = await ingestAndEnrich(env, league, dateISO);
  return enriched;
}
async function processRawEvents(rawEvents, env, league, dateISO) {
  if (rawEvents.length === 0) {
...
var init_fetchProps = __esm({
  "src/fetchProps.ts"() {
    ...
    __name(ingestAndEnrich, "ingestAndEnrich");
    __name(persistProps, "persistProps");
    ...
    __name(processRawEvents, "processRawEvents");
  }
});
    ...
    try {
      ...
      if (url.pathname === "/debug/sgo-api") {
        try {
          ...
          const getPlayerPropOddIDs = /* @__PURE__ */ __name((league2) => {
            const oddIDsMap = {
      ...
      if (url.pathname === "/debug/pure-worker") {
        try {
          ...
          console.log(`\u{1F9EA} Testing dual-mode approach for ${league} on ${date}...`);
      ...
      if (url.pathname === "/api/player-props") {
        try {
          ...
          try {
            if (date) {
              console.log(`\u{1F4CA} DUAL-MODE: Fetching and enriching props for ${league} on ${date}...`);
              enrichedProps = await ingestAndEnrich(env, league, date);
              if (enrichedProps.length > 0) {
                persistProps(env, enrichedProps).catch(
                  (err) => console.error("Background persist failed:", err)
                );
              }
              ...
              for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
                ...
                try {
                  console.log(`\u{1F4CA} DUAL-MODE: Fetching props for ${league} on ${dateStr}...`);
                  const dayProps = await ingestAndEnrich(env, league, dateStr);
                  if (dayProps.length > 0) {
                    persistProps(env, dayProps).catch(
                      (err) => console.error(`Background persist failed for ${dateStr}:`, err)
                    );
                  }
              ...
              for (let i = 0; i < 7; i++) {
                ...
                try {
                  ...
                  if (testProps.length > 0) {
                    console.log(`\u{1F4C5} DUAL-MODE: Found data for ${league} on ${dateStr} (${testProps.length} props)`);
                    enrichedProps = testProps;
                    foundData = true;
                    persistProps(env, testProps).catch(
                      (err) => console.error(`Background persist failed for ${dateStr}:`, err)
                    );
                    break;
                  }
