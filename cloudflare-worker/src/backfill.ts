...
export async function runBackfill(
  ...
  const { leagueId, season, days, batchSize, maxEventsPerDay } = {
  ...
  try {
    ...
    for (const event of events) {
      try {
        ...
        const eventProps = await extractPlayerPropsFromEvent(
          event,
          leagueId,
          season.toString(),
          undefined,
    ...
    console.log(
      `📊 ${leagueId}: Extracted ${allProps.length} props and ${allGameLogs.length} game logs`,
    );
    ...
    if (allProps.length > 0) {
      ...
      for (let i = 0; i < propChunks.length; i++) {
        try {
          ...
          console.log(
            `✅ ${leagueId}: Inserted props batch ${i + 1}/${propChunks.length} (${propChunks[i].length} props)`,
          );
        } catch (error) {
    ...
    if (allGameLogs.length > 0) {
      ...
      for (let i = 0; i < gameLogChunks.length; i++) {
        try {
          ...
          console.log(
            `✅ ${leagueId}: Inserted game logs batch ${i + 1}/${gameLogChunks.length} (${gameLogChunks[i].length} logs)`,
          );
        } catch (error) {
    ...
    console.log(
      `✅ ${leagueId} backfill complete: ${totalProps} props, ${totalGameLogs} game logs, ${totalErrors} errors`,
    );
...
export async function runMultiLeagueBackfill(
  env: any,
  days: number = 90,
): Promise<{
  totalProps: number;
  ...
  console.log(
    `📊 Found ${leagues.length} leagues for backfill: ${leagues.map((l) => l.id).join(", ")}`,
  );
  ...
  for (const league of leagues) {
    ...
    try {
      ...
      console.log(
        `✅ ${league.id}: ${result.propsInserted} props, ${result.gameLogsInserted} game logs, ${result.errors} errors`,
      );
    } catch (error) {
...
export async function runBackfillWithDateRange(
  env: any,
  config: {
    leagueId: string;
    season: number;
