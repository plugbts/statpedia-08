...
function normalizeName(
...
): { value: string; hadPropInName: boolean } {
  const original = name ?? "";
  ...
  if (lowerProp) {
    ...
    patterns.push(
      new RegExp(`\\s*-?\\s*${lowerProp.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&")}\\s*$`, "i"),
    );
    patterns.push(
      new RegExp(`^${lowerProp.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&")}\\s*-?\\s*`, "i"),
    );
  }
...
export function cleanPlayerNames(rows: RawPropRow[], logPrefix = "[worker:names]"): CleanPropRow[] {
  ...
  rows.forEach((row, idx) => {
    ...
    if (originalName && originalName.trim().length > 0) {
      ...
      const isPlayerIdFormat =
        /^[A-Z_]+_\d+_[A-Z]+$/.test(originalName) ||
    ...
    const finalName =
      !baseName || baseName.trim().length === 0 || /^[\W_]+$/.test(baseName)
        ? "Unknown Player"
        : baseName;
