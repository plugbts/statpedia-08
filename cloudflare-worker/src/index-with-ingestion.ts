...
async function handleAnalyticsStreaks(request: Request, env: Env): Promise<Response> {
  try {
    ...
    // Convert fetched logs into GameResult[] expected by calculateStreaks
    const gameResults = (gameLogs || []).map((g: any) => ({
      ...
      league: g.league,
      date: g.date,
      hit_result: typeof g.hit_result === "number" ? g.hit_result : g.hit ? 1 : 0,
    }));
