...
export async function runIngestion(env: any): Promise<IngestionResult> {
  ...
  console.log(
    `📊 Processing ${activeLeagues.length} active leagues: ${activeLeagues.map((l) => l.id).join(", ")}`,
  );
  ...
  for (const leagueConfig of activeLeagues) {
    ...
    try {
      ...
      totalErrors +=
        mappingStats.missingPlayerId + mappingStats.unmappedMarket + mappingStats.incompleteOdd;
      ...
      if (mappedProps.length > 0) {
        try {
          ...
          function logRowDiff(testRow: any, realRow: any) {
            ...
            console.log(
              "🔍 [SCHEMA DIFF] Missing in real:",
              testKeys.filter((k) => !(k in realRow)),
            );
            console.log(
              "🔍 [SCHEMA DIFF] Extra in real:",
              realKeys.filter((k) => !(k in testRow)),
            );

            for (const key of testKeys) {
              if (key in realRow && typeof testRow[key] !== typeof realRow[key]) {
                console.log(
                  `🔍 [SCHEMA DIFF] Type mismatch on ${key}: test=${typeof testRow[key]} (${testRow[key]}) vs real=${typeof realRow[key]} (${realRow[key]})`,
                );
              }
            }
          }
          ...
          if (insertResult.success) {
            ...
            console.log(
              `✅ ${leagueID}: Successfully inserted ${insertResult.proplinesInserted} proplines + ${insertResult.gameLogsInserted} game logs`,
            );
          } else {
            leagueErrors += insertResult.errors;
            console.error(
              `❌ ${leagueID}: Insert failed with ${insertResult.errors} errors:`,
              insertResult.errorDetails,
            );
          }
        } catch (error) {
      ...
      console.log(
        `✅ ${leagueID} ingestion complete: ${leagueInserted} inserted, ${leagueErrors} errors`,
      );
    } catch (error) {
  ...
  console.log(
    `📊 Total: ${totalProps} props processed, ${totalInserted} inserted, ${totalErrors} errors`,
  );
...
export async function runSingleLeagueIngestion(
  env: any,
  leagueId: string,
