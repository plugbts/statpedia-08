...
async function analyzeMismatches() {
  ...
  try {
    ...
    Object.entries(leagueAnalysis).forEach(([league, data]) => {
      ...
      console.log(`  Game Log Prop Types (${data.gameLogs.size}):`, Array.from(data.gameLogs).join(', '));
      ...
      console.log(`  Only in Game Logs (${onlyInGameLogs.size}):`, Array.from(onlyInGameLogs).join(', '));
      ...
      data.gameLogProps.forEach(gameLog => {
        const candidates = data.propProps.filter(prop => 
          prop.player_id === gameLog.player_id && 
          prop.game_id === gameLog.game_id
        );
