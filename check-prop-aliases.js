  ...
  try {
    ...
    if (aliasesError) {
      ...
      if (aliasesError.message.includes('relation "prop_type_aliases" does not exist')) {
        ...
        console.log('   SOLUTION: Create the prop_type_aliases table and populate it with proper mappings.');
      }
    } else {
    ...
    if (propTypesError) {
      ...
      const overUnderProps = uniquePropTypes.filter(p => 
      ...
      if (overUnderProps.length > 0) {
        ...
        console.log('   This confirms the issue - prop names are being stored as over/under instead of proper names.');
      }
    }
