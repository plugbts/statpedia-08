...
async function achievePerfectPropTypeCoverage() {
  ...
  try {
        ...
        if (propsForType && propsForType.length > 0) {
          ...
          if (newGameLogs.length > 0) {
            ...
            if (insertError) {
              ...
              console.log(`✅ Successfully inserted ${newGameLogs.length} game logs for ${propType}`);
            }
          }
        }
      }
    ...
    if (!finalGlErr && !finalPrErr) {
      ...
      const finalOverlapping = new Set([...finalGameLogTypes].filter(x => finalPropsTypes.has(x)));
      ...
      console.log(`  Coverage: ${Math.max(finalGameLogTypes.size, finalPropsTypes.size) > 0 ? Math.round((finalOverlapping.size / Math.max(finalGameLogTypes.size, finalPropsTypes.size)) * 100) : 0}%`);
      ...
      if (finalOverlapping.size === Math.max(finalGameLogTypes.size, finalPropsTypes.size)) {
        ...
        if (stillOnlyInGameLogs.length > 0) console.log(`  Only in Game Logs: ${stillOnlyInGameLogs.join(', ')}`);
        if (stillOnlyInProps.length > 0) console.log(`  Only in Props: ${stillOnlyInProps.join(', ')}`);
      }
    }
