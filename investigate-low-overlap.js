...
async function investigateLowOverlap() {
  ...
  try {
    ...
    if (gameLogsError || propLinesError) {
      ...
      console.log(`✅ Overlap: ${gameLogsUnique.filter(id => propLinesUnique.includes(id)).length} players`);
      
      // Show the actual overlap percentage
      const overlapPercentage = (gameLogsUnique.filter(id => propLinesUnique.includes(id)).length / gameLogsUnique.length * 100).toFixed(1);
    ...
    if (gameLogsLeaguesError || propLinesLeaguesError) {
      console.error('❌ Error fetching league data:', gameLogsLeaguesError || propLinesLeaguesError);
    } else {
    ...
    if (gameLogsCountError || propLinesCountError) {
      ...
      if (propLinesPlayers > 0) {
        console.log(`🎯 Avg Prop Lines per Player: ${(propLinesCount / propLinesPlayers).toFixed(1)}`);
      }
    }
    ...
    for (const playerName of commonPlayers) {
      ...
      if (gameLogsPlayerError || propLinesPlayerError) {
        console.error(`❌ Error checking ${playerName}:`, gameLogsPlayerError || propLinesPlayerError);
      } else {
        const hasGameLogs = gameLogsPlayer?.length > 0;
        const hasPropLines = propLinesPlayer?.length > 0;
        
        console.log(`${playerName}:`);
        console.log(`  Game Logs: ${hasGameLogs ? '✅' : '❌'} (${gameLogsPlayer?.length || 0} records)`);
        console.log(`  Prop Lines: ${hasPropLines ? '✅' : '❌'} (${propLinesPlayer?.length || 0} records)`);
