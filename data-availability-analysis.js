  ...
  try {
    ...
    for (const league of activeLeagues) {
      ...
      if (!glCountErr && !prCountErr) {
        ...
        console.log(`    Status: ${gameLogCount > 0 && propsCount > 0 ? '✅ Data Available' : '❌ No Data'}`);
      }
    }
    ...
    if (!dateErr && dateRange && dateRange.length > 0) {
      const dates = dateRange.map(d => d.date_normalized || d.date?.split('T')[0]).filter(Boolean);
    ...
    const onlyMLB = gameLogLeagues.size === 1 && gameLogLeagues.has('mlb') && 
                    propsLeagues.size === 1 && propsLeagues.has('mlb');
