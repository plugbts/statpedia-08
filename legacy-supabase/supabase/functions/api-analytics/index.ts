...
class APIAnalyticsService {
  ...
  async getAnalytics(
    ...
    const avgResponseTime =
      logs.reduce((sum, log) => sum + (log.response_time_ms || 0), 0) / requestCount;
  ...
  async getSystemHealth(): Promise<any> {
    ...
    // Get API config
    const { data: config } = await supabase.from("api_config").select("*");
  ...
  async cleanupOldData(): Promise<{ cleaned: number }> {
    ...
    const totalCleaned =
      (deletedLogs?.length || 0) + (deletedCache?.length || 0) + (deletedRateLimits?.length || 0);
...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      case "analytics":
        const analytics = await service.getAnalytics(startDate || undefined, endDate || undefined);
