  ...
  do {
    try {
      ...
      if (data.success && data.data) {
        ...
        console.log(
          `Fetched ${data.data.length} events (page ${pageCount}, total: ${allEvents.length})`,
        );
      } else {
        break;
      }
...
async function extractPlayerPropsFromEvent(
  ...
  console.log(
    `✅ Processed ${props.length} props out of ${totalOdds} total odds for event ${event.eventID}`,
  );
  return props;
}
...
async function createPlayerPropsFromOddDebug(
  odd: any,
  oddId: string,
  event: any,
  league: string,
  ...
  try {
    ...
    console.log(
      `✅ Processed ${validProps.length} valid props out of ${props.length} total for odd ${oddId}`,
    );
    return validProps;
  } catch (err) {
    console.error(
      "❌ Exception in createPlayerPropsFromOddDebug:",
      err,
      "OddId:",
      oddId,
      "Odd:",
...
async function upsertProps(
  ...
  for (const prop of props) {
    try {
      ...
      const { error } = await supabaseClient.from("proplines").insert({
        ...prop,
...
async function logIngestionStats(supabaseClient: any, stats: any) {
  try {
    await supabaseClient.from("debug_ingestion_stats").insert({
...
async function healthCheck(supabaseClient: any) {
  try {
    ...
    const apiResponse = await fetch(
      `${SPORTSGAMEODDS_BASE_URL}/v2/events?sportID=FOOTBALL&limit=1`,
      {
        headers: {
          Accept: "application/json",
          "User-Agent": "Statpedia/1.0",
