...
class SportGameOddsAPIService {
  ...
  async fetchFromSportGameOdds(endpoint: string, sport?: string): Promise<any> {
    ...
    console.log(
      `Fetching from SportGameOdds API: ${url.replace(config.sportsgameodds_api_key, "REDACTED")}`,
    );
    ...
    try {
      ...
      console.log(
        `✅ SportGameOdds API response: ${data?.data?.length || 0} events (${totalSize} bytes, ${responseTime}ms)`,
      );
  ...
  async processPlayerProps(rawData: any, maxProps: number): Promise<any[]> {
    ...
    console.log(
      `🎯 Memory-optimized processing: ${maxEvents} events (limited from ${rawData.data.length}) for max ${maxProps} props`,
    );
      ...
      for (const [oddID, oddData] of Object.entries(event.odds)) {
        ...
        for (const [bookmakerId, bookmakerData] of Object.entries(odd.byBookmaker)) {
          ...
          if (!prop) {
            ...
            const determinedTeamAbbr = this.determinePlayerTeam(
              playerIdPart,
              homeTeamAbbr,
              awayTeamAbbr,
            );
          ...
          if (side === "over") {
            ...
            if (
              prop.underOdds === null ||
              this.isBetterOdds(newUnderOdds, prop.underOdds, "under")
            ) {
          ...
          const existingBookmaker = prop.allSportsbookOdds.find(
            (sb) => sb.sportsbook === bookmakerId,
          );
          ...
          if (
            ...
            new Date(bookmaker.lastUpdatedAt) > new Date(prop.lastUpdate)
          ) {
    ...
    let playerProps = Array.from(playerPropsMap.values()).filter(
      (prop) => prop.overOdds !== null && prop.underOdds !== null,
    ); // Only include props with both odds
    ...
    playerProps = playerProps.map((prop) => ({
      ...
      confidence: this.calculateConfidenceFromOdds(
        prop.overOdds,
        prop.underOdds,
        prop.availableSportsbooks.length,
      ),
    }));
    ...
    console.log(
      `Processed ${playerProps.length} complete player props with real odds and calculated metrics`,
    );
    return playerProps;
  }
  ...
  private calculateConfidenceFromOdds(
    overOdds: number,
    underOdds: number,
  ...
  private normalizeTeamName(teamName: string): string {
    ...
    const cleaned = normalized.replace(/\b(team|fc|club|city|the)\b/gi, "").trim();
  ...
  private parseAmericanOdds(oddsValue: any): number {
    console.log(`🎲 Parsing odds:`, {
      oddsValue,
      type: typeof oddsValue,
      raw: JSON.stringify(oddsValue),
    });
...
serve(async (req) => {
  ...
  try {
    ...
    if (!config.sportsgameodds_api_key) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error:
            "SportGameOdds API key not configured. Please set the API key in the database or environment variables.",
        }),
        {
          status: 500,
          headers: {
    ...
    console.log(
      `${forceRefresh ? "Force refresh" : "Cache miss"} for ${cacheKey}, fetching fresh data from API`,
    );
