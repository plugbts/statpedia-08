 ...
 * Runs the complete data pipeline every day at midnight UTC
 */
...
async function runNightlyJob() {
  ...
  const results = {
    gameLogs: { success: false, records: 0, leagues: {} },
  ...
  try {
    ...
    const gameLogResults = await ingestGameLogs();
    results.gameLogs = gameLogResults;

    // Step 2: Ingest PropLines
    console.log("\n🎯 STEP 2: Ingest PropLines");
    console.log("-".repeat(40));
    const propLineResults = await ingestPropLines();
    results.propLines = propLineResults;
    ...
    // Final Summary
    console.log("\n🎉 SCHEDULED NIGHTLY JOB COMPLETE");
    console.log("=".repeat(60));
    ...
    console.log("\n✅ Scheduled nightly job completed successfully!");

    return results;
  } catch (error) {
    console.error("❌ Scheduled nightly job failed:", error);
    results.totalTime = Date.now() - startTime;
    console.log(`⏱️ Total execution time: ${(results.totalTime / 1000).toFixed(2)}s`);
    throw error;
  }
...
async function ingestGameLogs() {
  ...
  const ingestionResults = { success: true, records: 0, leagues: {} };
  ...
  for (const league of LEAGUES) {
    ...
    do {
      try {
        ...
        if (!res.ok) {
          ...
          ingestionResults.success = false;
          break;
        }
        ...
        if (rows.length > 0) {
          const { error } = await supabase.from("playergamelogs").upsert(rows);
    ...
    ingestionResults.leagues[league] = leagueRecords;
    ingestionResults.records += leagueRecords;
...
/* ------------------------------
   2. Ingest PropLines (from events odds data)
--------------------------------*/
  ...
  for (const league of LEAGUES) {
    ...
    do {
      try {
        ...
        if (rows.length > 0) {
          const { error } = await supabase.from("proplines").upsert(rows);
...
async function precomputeAnalytics(season) {
  ...
  try {
    ...
    if (!combos || combos.length === 0) {
      console.log("⚠️ No player/prop combinations found for analytics");
      return { records: 0, success: true };
    }

    // Create unique combinations map
    ...
    if (results.length > 0) {
      console.log(
        `💾 Upserting ${results.length} analytics records into player_analytics table...`,
      );

      // Log first few records as examples
      results.slice(0, 3).forEach((record, i) => {
        console.log(
          `  ${i + 1}. ${record.player_name} ${record.prop_type} ${record.direction} ${record.line} - Season: ${record.season_pct?.toFixed(1)}% (${record.season_hits}/${record.season_total})`,
        );
      });
      ...
      const { error: upsertError } = await supabase.from("player_analytics").upsert(results, {
...
GRANT USAGE ON SEQUENCE player_analytics_id_seq TO authenticated;
        ...
        console.log(
          `✅ Successfully upserted ${results.length} analytics records into player_analytics table`,
        );
        return { records: results.length, success: true };
      }
    }
...
async function createAnalyticsTableIfNotExists() {
  ...
  try {
    ...
    if (error) {
      ...
      const { error: insertError } = await supabase.from("player_analytics").insert([
        {
          player_id: "dummy",
          player_name: "dummy",
          prop_type: "dummy",
      ...
      if (
        insertError &&
        insertError.message.includes('relation "player_analytics" does not exist')
      ) {
        console.log("❌ Table does not exist and could not be created automatically");
        console.log(
          "💡 Please run the migration manually or create the table in Supabase dashboard",
        );
      } else if (insertError) {
...
console.log("🕐 Scheduling nightly job to run at 00:00 UTC every day...");
console.log('📅 Cron pattern: "0 0 * * *" (every day at midnight)');

cron.schedule("0 0 * * *", () => {
  console.log("⏰ Nightly job triggered at", new Date().toISOString());
  console.log("\n⏰ CRON TRIGGERED - Starting scheduled nightly job...");
  runNightlyJob().catch((error) => {
...
console.log("🚀 Running initial job for testing...");
runNightlyJob().catch((error) => {
  console.error("❌ Initial job failed:", error);
  process.exit(1);
});

console.log("✅ Scheduler started successfully!");
console.log("📊 The job will run automatically every day at midnight UTC");
