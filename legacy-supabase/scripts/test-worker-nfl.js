...
async function testWorkerNFL() {
  ...
  try {
    ...
    const receivingYards = nflProps.filter(
      (prop) => prop.prop_type && prop.prop_type.toLowerCase().includes("receiving"),
    );
    ...
    const comboProps = nflProps.filter(
      (prop) => prop.prop_type && (prop.prop_type.includes("+") || prop.prop_type.includes("_")),
    );
