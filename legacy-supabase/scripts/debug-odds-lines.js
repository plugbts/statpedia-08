...
async function debugOddsAndLines() {
  ...
  try {
    ...
    props?.forEach((prop, index) => {
      ...
      if (
        prop.over_odds &&
        (prop.over_odds === 0 || prop.over_odds < -1000 || prop.over_odds > 1000)
      ) {
      ...
      if (
        prop.under_odds &&
        (prop.under_odds === 0 || prop.under_odds < -1000 || prop.under_odds > 1000)
      ) {
    ...
    if (lineStats) {
      ...
      console.log(
        `Integer lines: ${integerLines.length} (${((integerLines.length / lineStats.length) * 100).toFixed(1)}%)`,
      );
      console.log(
        `Decimal lines: ${decimalLines.length} (${((decimalLines.length / lineStats.length) * 100).toFixed(1)}%)`,
      );
    ...
    if (oddsStats) {
      ...
      console.log(
        `Valid over odds: ${validOverOdds.length} (${((validOverOdds.length / oddsStats.length) * 100).toFixed(1)}%)`,
      );
      console.log(
        `Valid under odds: ${validUnderOdds.length} (${((validUnderOdds.length / oddsStats.length) * 100).toFixed(1)}%)`,
      );
      console.log(
        `Missing odds: ${missingOdds.length} (${((missingOdds.length / oddsStats.length) * 100).toFixed(1)}%)`,
      );
