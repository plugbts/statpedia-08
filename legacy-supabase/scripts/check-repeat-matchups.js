    ...
    if (repeatError) {
      ...
      if (repeatMatchups.length > 0) {
        ...
        repeatMatchups
          ...
          .forEach((matchup, i) => {
            console.log(
              `${i + 1}. ${matchup.player_name} vs ${matchup.opponent} (${matchup.league}, ${matchup.prop_type}): ${matchup.count} games`,
            );
            console.log(
              `   Dates: ${matchup.dates.slice(0, 3).join(", ")}${matchup.dates.length > 3 ? "..." : ""}`,
            );
          });
