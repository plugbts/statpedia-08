...
const supabase = createClient(
  ...
  process.env.SUPABASE_SERVICE_KEY ||
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InJmZHJpZnJzZm9icWx6b3JjZXNuIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1OTA0ODgzNiwiZXhwIjoyMDc0NjI0ODM2fQ.E9LQFvqVs9Z1cXoS34ov5qV3jBkbqCHPCL16GBt480g",
);
...
class BackfillMonitor {
  ...
  async checkProgress() {
    try {
      ...
      console.log(
        `   player_game_logs: ${currentCounts.logs.toLocaleString()} records (${logsProgress}%)`,
      );
      console.log(
        `   proplines: ${currentCounts.props.toLocaleString()} records (${propsProgress}%)`,
      );
      ...
      if (
        currentCounts.logs >= this.targetRecords.logs * 0.95 &&
  ...
  async checkProcessStatus() {
    try {
      ...
      const totalGrowth =
        this.lastCounts.logs > 0
          ? this.lastCounts.logs +
            this.lastCounts.props -
  ...
  async startMonitoring() {
    ...
    console.log(
      `📊 Target: ~${this.targetRecords.logs.toLocaleString()} records across both tables`,
    );
    ...
    console.log(
      `⏱️ Total monitoring time: ${((Date.now() - this.startTime) / (1000 * 60)).toFixed(1)} minutes`,
    );
  }
}
