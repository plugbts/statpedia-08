...
async function fixApiIngestion() {
  ...
  try {
    ...
    const v2NoSeasonResponse = await fetch(
      `https://api.sportsgameodds.com/v2/events?apiKey=${API_KEY}&oddsAvailable=true&leagueID=NFL&limit=100`,
    );
    ...
    const v2Season2024Response = await fetch(
      `https://api.sportsgameodds.com/v2/events?apiKey=${API_KEY}&oddsAvailable=true&leagueID=NFL&season=2024&limit=100`,
    );
    ...
    const eventsResponse = await fetch(
      "https://api.sportsgameodds.com/events?league=nfl&limit=10",
      {
        headers: { "x-api-key": API_KEY },
      },
    );
    ...
    const moreEventsResponse = await fetch(
      "https://api.sportsgameodds.com/events?league=nfl&limit=200",
      {
        headers: { "x-api-key": API_KEY },
      },
    );
    ...
    if (moreEventsResponse.ok) {
      ...
      for (const event of moreEventsData.data || []) {
        if (event.status?.completed && event.results?.game) {
          ...
          const players = Object.keys(event.results.game).filter(
