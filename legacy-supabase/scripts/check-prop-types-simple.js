...
async function checkPropTypesSimple() {
  ...
  try {
    ...
    const receivingProps = Object.keys(propTypeGroups).filter(
      (type) =>
    ...
    if (receivingProps.length > 0) {
      receivingProps.forEach((propType) => {
        ...
        props.slice(0, 3).forEach((prop) => {
          console.log(
            `     ${prop.player_name}: ${prop.line} (Over: ${prop.over_odds}, Under: ${prop.under_odds})`,
          );
        });
      });
    } else {
    ...
    const comboProps = Object.keys(propTypeGroups).filter(
      (type) =>
        type.includes("+") ||
        type.includes("&") ||
    ...
    const missingUnderOdds = nflProps.filter(
      (prop) => prop.under_odds === null || prop.under_odds === undefined,
    );
    ...
    if (missingUnderOdds.length > 0) {
      ...
      missingUnderOdds.slice(0, 10).forEach((prop) => {
        console.log(
          `     ${prop.player_name}: ${prop.prop_type} ${prop.line} (Over: ${prop.over_odds}, Under: ${prop.under_odds})`,
        );
      });
    } else {
    ...
    const missingOverOdds = nflProps.filter(
      (prop) => prop.over_odds === null || prop.over_odds === undefined,
    );
    ...
    console.log(
      '4. The frontend overUnderFilter="both" should show props with either over OR under odds',
    );
  } catch (error) {
