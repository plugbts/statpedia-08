...
async function createSyntheticGameLogs() {
  ...
  try {
    ...
    Object.entries(propsByLeague).forEach(([league, props]) => {
      ...
      uniqueCombinations.forEach((combination) => {
        ...
        const matchingProp = props.find(
          (p) => p.player_id === player_id && p.game_id === game_id && p.prop_type === prop_type,
        );
    ...
    for (let i = 0; i < syntheticGameLogs.length; i += batchSize) {
      ...
      const { error: insertError } = await supabase.from("player_game_logs").insert(batch);
      ...
      if (insertError) {
        ...
        console.log(
          `✅ Inserted batch ${Math.floor(i / batchSize) + 1}: ${batch.length} game logs`,
        );
      }
    }
