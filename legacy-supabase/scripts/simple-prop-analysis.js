...
async function simplePropAnalysis() {
  ...
  try {
    ...
    Object.values(propTypeStats).forEach((stat) => {
      ...
      console.log(
        `   ${stat.prop_type}: ${stat.lines.length} props, line range ${minLine}-${maxLine}, leagues: ${leagues}`,
      );
    });
    ...
    Object.values(suggestionGroups).forEach((group) => {
      ...
      console.log(
        `   ${group.prop} (${group.league}): lines ${minLine}-${maxLine} (${group.lines.length} props)`,
      );
    });
