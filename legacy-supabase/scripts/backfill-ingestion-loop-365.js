...
async function fetchGamesForDate(date, league) {
  ...
  console.log(
    `📊 Extracted ${performanceData.length} performance records for ${league} on ${date}`,
  );
...
async function backfillParallel(league, daysBack, batchSize = 5) {
  ...
  console.log(`🚀 Starting 365-day backfill for ${league} - ${daysBack} days back`);
  ...
  console.log(`✅ Completed 365-day backfill for ${league}`);
}

// Run backfill for 365 days
(async () => {
  try {
    console.log("🚀 Starting comprehensive 365-day backfill for all leagues...");

    // Run backfill for each league - 365 days
    await backfillParallel("nhl", 365, 10); // last 365 days, 10 days per batch
    await backfillParallel("nba", 365, 10);
    await backfillParallel("mlb", 365, 10);
    await backfillParallel("nfl", 365, 10);

    console.log("✅ All 365-day backfills completed successfully!");

    // Run diagnostic query to check for H2H data
    console.log("🔍 Running final diagnostic query...");
    ...
    if (logsError || propsError) {
      ...
      console.log("📊 Final Backfill Results Summary:");
      console.log(`- player_game_logs: ${totalLogs || 0} records`);
      console.log(`- proplines: ${totalProps || 0} records`);
      console.log("✅ 365-day historical data backfill completed!");
      console.log("\n🔍 Now run: node check-repeat-matchups.js");
    }
  } catch (error) {
    console.error("❌ 365-day backfill failed:", error);
  }
})();
