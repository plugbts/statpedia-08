...
async function expandLeagueCoverage() {
  ...
  try {
    ...
    const allLeagues = new Set([...Object.keys(gameLogLeagues), ...Object.keys(propLeagues)]);
    ...
    allLeagues.forEach((league) => {
      ...
      console.log(
        `  ${league.toUpperCase()}: ${coverage} overlapping prop types (${hasGameLogs} logs, ${hasProps} props)`,
      );
    });

    // 3. Create synthetic data for missing leagues
    const targetLeagues = ["nfl", "nba", "mlb", "nhl"];
    const missingLeagues = targetLeagues.filter(
      (league) => !gameLogLeagues[league] || gameLogLeagues[league].size === 0,
    );
    ...
    for (const league of Object.keys(gameLogLeagues)) {
      if (gameLogLeagues[league].size > 0) {
        ...
        if (additionalProps.length > 0) {
          const { error: insertError } = await supabase.from("proplines").insert(additionalProps);
