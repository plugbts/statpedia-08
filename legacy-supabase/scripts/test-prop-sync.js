...
async function testPropTypeSync() {
  ...
  try {
    ...
    testCases.forEach((test) => {
      ...
      console.log(
        `${status} ${test.input} → ${result} (${test.league}) ${result === test.expected ? "" : `(expected: ${test.expected})`}`,
      );
    });
    ...
    const { data: props } = await supabase.from("proplines").select("prop_type, league").limit(10);
