...
export async function runAlignmentHealthCheck(supabaseUrl, supabaseKey) {
  ...
  try {
    ...
    console.log(
      `   Proplines: ${proplinesDates?.[0]?.date || "N/A"} to ${proplinesDatesMax?.[0]?.date || "N/A"}`,
    );
    console.log(
      `   Game Logs: ${gameLogsDates?.[0]?.date || "N/A"} to ${gameLogsDatesMax?.[0]?.date || "N/A"}`,
    );
    ...
    console.log(
      `   Proplines prop types (${proplinesPropTypes.length}):`,
      proplinesPropTypes.slice(0, 5),
    );
    console.log(
      `   Game Logs prop types (${gameLogsPropTypes.length}):`,
      gameLogsPropTypes.slice(0, 5),
    );
    ...
    console.log(
      "   Proplines player samples:",
      proplinesPlayers?.map((p) => `${p.player_id} (${p.player_name})`),
    );
    console.log(
      "   Game Logs player samples:",
      gameLogsPlayers?.map((p) => `${p.player_id} (${p.player_name})`),
    );
    ...
    if ((recentProplines?.length || 0) === 0)
      issues.push("❌ No recent proplines data (last 7 days)");
    if ((recentGameLogs?.length || 0) === 0)
      issues.push("❌ No recent game logs data (last 7 days)");
