...
async function testRPCConstraints() {
  ...
  try {
    ...
    const { data: sampleProplinesResult, error: sampleProplinesError } = await supabase.rpc(
      "bulk_upsert_proplines",
      {
    ...
    const { data: sampleGameLogsResult, error: sampleGameLogsError } = await supabase.rpc(
      "bulk_upsert_player_game_logs",
      {
