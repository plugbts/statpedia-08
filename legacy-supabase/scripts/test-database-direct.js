...
async function testDatabaseDirect() {
  ...
  try {
    ...
    const {
      data: proplinesData,
      error: proplinesError,
      count: proplinesCount,
    } = await supabase.from("proplines").select("*", { count: "exact" }).limit(5);
    ...
    const {
      ...
      error: gameLogsError,
      count: gameLogsCount,
    } = await supabase.from("player_game_logs").select("*", { count: "exact" }).limit(5);
