...
async function runDiagnostics() {
  ...
  gameLogs?.forEach((g) => {
    ...
    if (match) {
      ...
      mismatched++;
      const issues = [];
      if (g.conflict_key !== candidates[0].conflict_key)
        issues.push(`conflict_key: "${g.conflict_key}" vs "${candidates[0].conflict_key}"`);
      if (g.date !== candidates[0].date_normalized)
        issues.push(`date: "${g.date}" vs "${candidates[0].date_normalized}"`);
      if (g.prop_type !== candidates[0].prop_type)
        issues.push(`prop_type: "${g.prop_type}" vs "${candidates[0].prop_type}"`);
      if (g.league.toLowerCase() !== candidates[0].league.toLowerCase())
        issues.push(`league: "${g.league}" vs "${candidates[0].league}"`);
