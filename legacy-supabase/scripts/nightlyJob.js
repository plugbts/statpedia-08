      ...
      try {
        ...
        for (const event of data.data || []) {
          ...
          for (const [playerId, playerStats] of Object.entries(event.results.game)) {
            ...
            const canonicalPlayerId = await mapPlayerId(
              "logs",
              playerId,
              playerName,
              normalizedTeam,
  ...
  try {
    ...
    for (const { player_id, player_name, prop_type } of uniqueCombinations.values()) {
      try {
        ...
        if (!joined || joined.length === 0) {
          console.log(
            `  ⚠️ Skipping ${player_name} ${prop_type} - no matching prop lines found (${gameLogs?.length || 0} game logs, ${propLines?.length || 0} prop lines)`,
          );
          continue;
        }
        ...
        console.log(
          `  ✅ Creating analytics for ${player_name} ${prop_type} - ${joined.length} joined records`,
        );
        results.push({
          player_id,
          player_name,
    ...
    if (results.length > 0) {
      ...
      results.slice(0, 3).forEach((record, i) => {
        console.log(
          `  ${i + 1}. ${record.player_name} ${record.prop_type} - Season ${record.season}`,
        );
      });
      ...
      const { error: upsertError } = await supabase.from("playeranalytics").upsert(results, {
...
GRANT USAGE ON SEQUENCE playeranalytics_id_seq TO authenticated;
        ...
        console.log(
          `✅ Successfully upserted ${results.length} analytics records into playeranalytics table`,
        );
