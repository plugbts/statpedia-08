...
async function analyzeAllLeaguesProps() {
  ...
  try {
      ...
      if (mapping) {
        mapping.ranges.forEach((range) => {
          ...
          if (matchingLines.length > 0) {
            console.log(
              `   Line ${range.min}-${range.max}: ${matchingLines.length} props → ${range.prop} (${range.desc})`,
            );
          }
        });
      } else {
        ...
        console.log(
          `   Lines: ${uniqueLines.slice(0, 15).join(", ")}${uniqueLines.length > 15 ? "..." : ""}`,
        );
      }
    });
    ...
    const totalOverUnder = Object.values(leagueStats).reduce(
      (sum, stats) => sum + stats.overUnderCount,
      0,
    );
    ...
    Object.entries(leagueStats).forEach(([league, stats]) => {
      ...
      console.log(
        `   ${league}: ${stats.overUnderCount}/${stats.totalProps} over/under (${percentage}%)`,
      );
    });
