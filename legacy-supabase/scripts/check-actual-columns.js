...
async function checkActualColumns() {
  ...
  try {
    const { data, error } = await supabase.from("playeranalytics").insert([minimalRecord]).select();
    ...
    if (error) {
      ...
      if (error.message.includes("column")) {
        ...
        for (const col of basicColumns) {
          try {
            const { error: colError } = await supabase.from("playeranalytics").select(col).limit(1);
