...
async function debugCurrentIngestion() {
  ...
  try {
    ...
    if (recentProps.length === 0) {
      ...
      if (olderProps.length > 0) {
        ...
        olderProps.forEach((prop, index) => {
          console.log(`${index + 1}. ${prop.player_name} | ${prop.prop_type} | ${prop.line} | ${prop.over_odds}/${prop.under_odds} | ${prop.created_at}`);
        });
      }
      return;
    }
    ...
    const problematicProps = recentProps.filter(prop => 
    ...
    if (problematicProps.length > 0) {
      ...
      problematicProps.forEach((prop, index) => {
        console.log(`${index + 1}. ${prop.player_name} | ${prop.prop_type} | ${prop.line} | ${prop.over_odds}/${prop.under_odds}`);
      });
