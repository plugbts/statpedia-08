-- Fixed API Functions with Resolved Ambiguous Column References
-- This fixes the ambiguous column reference issues
...
BEGIN
    ...
    FOR func_record IN 
        ...
        AND (routine_name LIKE '%api_usage%' OR routine_name LIKE '%log_api%' OR routine_name LIKE '%sportsgameodds%')
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS %I CASCADE', func_record.specific_name);
    END LOOP;
END $$;

-- Step 2: Create log_sportsgameodds_api_usage function with fixed ambiguous references
...
DECLARE
  ...
  v_cost_per_request DECIMAL(8,6);
  v_cache_discount DECIMAL(5,2);
  v_request_cost DECIMAL(10,4);
BEGIN
  ...
  INTO v_cost_per_request, v_cache_discount
  ...
  v_request_cost := v_cost_per_request;
  IF p_cache_hit THEN
    v_request_cost := v_request_cost * (1 - v_cache_discount / 100);
  END IF;
  ...
  -- Update current usage summary with explicit table references
  INSERT INTO public.api_current_usage (
    ...
    p_user_id, v_current_month_start, 1, p_response_time_ms,
  ...
  ON CONFLICT (user_id, current_month_start) DO UPDATE SET
    ...
    END,
    estimated_cost_usd = api_current_usage.estimated_cost_usd + v_request_cost,
    updated_at = now();
    
  -- Update monthly summary with explicit table references
  ...
  ON CONFLICT (user_id, current_month_start) DO UPDATE SET
    ...
    END,
    estimated_cost_usd = api_usage_summary.estimated_cost_usd + v_request_cost,
    updated_at = now();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Step 3: Create get_sportsgameodds_api_usage_stats function
...
-- Step 4: Create get_sportsgameodds_api_usage_vs_plan function
...
-- Step 6: Test the functions with explicit type casts
SELECT 'Testing fixed functions with explicit type casts...' as step;
