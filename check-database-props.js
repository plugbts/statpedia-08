...
async function checkDatabaseProps() {
  ...
  try {
    ...
    const { data: problematicProps, error: probError } = await supabase
      ...
      .or('prop_type.eq.Over/Under,prop_type.eq.over_under,prop_type.ilike.%receivingeptions%,prop_type.eq.unknown')
      .order('count', { ascending: false });
    ...
    if (problematicProps && problematicProps.length > 0) {
      ...
      console.log('   Copy and paste the contents of ultra-safe-cleanup.sql into your Supabase SQL Editor');
    } else {
    ...
    if (recentProps && recentProps.length > 0) {
      ...
      recentProps.forEach((prop, index) => {
        console.log(`${index + 1}. ${prop.player_name} | ${prop.prop_type} | ${prop.line} | ${prop.created_at}`);
      });
      
      // Check if recent props have clean prop types
      const cleanRecentProps = recentProps.filter(p => 
      ...
      console.log('   This might mean the worker isn\'t ingesting new data, or there\'s a timezone issue');
    }
