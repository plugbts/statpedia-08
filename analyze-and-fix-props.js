...
async function analyzeAndFixProps() {
  ...
  try {
    ...
    Object.values(propTypeStats).forEach(stat => {
      ...
      console.log(`   ${stat.prop_type} (${stat.sport}): ${stat.count} props, line range ${minLine}-${maxLine}`);
    });
    ...
    fixSQL += 'SELECT prop_type, sport, COUNT(*) as count, MIN(line) as min_line, MAX(line) as max_line\n';
    ...
    if (overUnderError) {
      ...
      overUnderProps?.forEach(prop => {
        const mapping = findBestMapping(prop.line, prop.sport, nflMappings, nbaMappings, nhlMappings);
