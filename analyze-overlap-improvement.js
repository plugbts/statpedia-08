  ...
  try {
    ...
    if (overlapError) {
      ...
      overlapPlayers?.forEach((player, i) => {
        console.log(`${i + 1}. ${player.player_name} - ${player.prop_type} (ID: ${player.player_id})`);
      });
    }
    ...
    if (unmappedGameLogsError) {
      ...
      console.log(`📊 Game logs players that might need mapping (${unmappedGameLogs?.length || 0}):`);
    ...
    if (unmappedPropLinesError) {
      ...
      console.log(`🎯 Prop lines players that might need mapping (${unmappedPropLines?.length || 0}):`);
