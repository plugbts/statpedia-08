-- API Functions Setup (Run AFTER api-tables-setup.sql)
-- This creates the functions that depend on the tables
...
-- Step 2: Create log_api_usage function
...
-- Step 3: Create get_api_usage_stats function
...
DECLARE
  default_cost DECIMAL(8,6) := 0.001; -- Default cost if no plan config
BEGIN
  RETURN QUERY
  SELECT 
    logs.user_id,
...
-- Step 4: Create get_api_usage_vs_plan function
...
SELECT 'Testing API functions...' as step;
