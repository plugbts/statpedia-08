...
async function runBackfill() {
  ...
  try {
    ...
    console.log('=' .repeat(80));
    console.log(`📅 Seasons processed: ${Object.keys(totalResults.seasons).length}/${SEASONS.length}`);
  ...
  for (const league of LEAGUES) {
    ...
    do {
      try {
        ...
        if (!res.ok) {
          console.error(`❌ API request failed for ${league} ${season}: ${res.status} ${res.statusText}`);
          break;
        }
    ...
    console.log(`✅ ${league.toUpperCase()} ${season}: ${leagueRecords.toLocaleString()} game log records`);
  }
...
async function ingestSeasonProps(season) {
  ...
  for (const league of LEAGUES) {
    ...
    results.leagues[league] = leagueRecords;
    results.records += leagueRecords;
    console.log(`✅ ${league.toUpperCase()} ${season}: ${leagueRecords.toLocaleString()} prop line records`);
  }
  ...
  try {
    ...
    for (const { player_id, player_name, prop_type } of uniqueCombinations.values()) {
      try {
        ...
        processed++;
        if (processed % 50 === 0) {
          console.log(`  📊 Processed ${processed.toLocaleString()}/${uniqueCombinations.size.toLocaleString()} combinations`);
        }
    ...
    if (results.length > 0) {
      ...
      if (upsertError) {
        ...
        console.log(`✅ Successfully upserted ${results.length.toLocaleString()} analytics records`);
