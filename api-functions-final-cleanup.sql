-- Complete API Functions Cleanup and Recreation
-- This removes ALL existing functions and recreates them with unique signatures

-- Step 1: Drop ALL existing API functions (comprehensive cleanup)
DO $$ 
DECLARE
    func_record RECORD;
BEGIN
    -- Find all functions that start with api_usage or contain api_usage
    FOR func_record IN 
...
-- Step 3: Create log_api_usage function with explicit signature
...
-- Step 4: Create get_api_usage_stats function with explicit signature
...
-- Step 5: Create get_api_usage_vs_plan function with explicit signature
CREATE OR REPLACE FUNCTION public.get_api_usage_vs_plan(
  p_user_id UUID DEFAULT NULL
)
RETURNS TABLE (
  user_id UUID,
...
GRANT EXECUTE ON FUNCTION public.log_api_usage(UUID, TEXT, TEXT, TEXT, INTEGER, INTEGER, BOOLEAN, TEXT, TEXT, INET) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_api_usage_stats(UUID, TIMESTAMP WITH TIME ZONE, TIMESTAMP WITH TIME ZONE) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_api_usage_vs_plan(UUID) TO authenticated;
...
-- Step 8: Test functions with explicit calls
SELECT 'Testing functions with explicit parameters...' as step;

-- Test with explicit parameter types
SELECT * FROM public.get_api_usage_stats(NULL::UUID, (now() - interval '30 days')::TIMESTAMP WITH TIME ZONE, now()::TIMESTAMP WITH TIME ZONE);
...
SELECT * FROM public.get_api_usage_vs_plan();
