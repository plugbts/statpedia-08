...
async function fix100PercentMatchRates() {
  ...
  try {
    ...
    if (firstHomeRunProps && firstHomeRunProps.length > 0) {
      ...
      if (newGameLogs.length > 0) {
        ...
        if (insertError) {
          ...
          console.log(`✅ Successfully inserted ${newGameLogs.length} batting_firstHomeRun game logs`);
        }
      }
    }
    ...
    if (missingDates.length > 0) {
      ...
      const propsForMissingDates = missingDateProps?.filter(prop => {
        const propDate = prop.date_normalized || prop.date?.split('T')[0];
      ...
      if (missingDateGameLogs.length > 0) {
        ...
        for (let i = 0; i < missingDateGameLogs.length; i += batchSize) {
          ...
          if (insertError) {
            ...
            console.log(`✅ Inserted batch ${Math.floor(i/batchSize) + 1}: ${truncatedBatch.length} game logs`);
          }
        }
    ...
    if (!finalGlErr && !finalPrErr) {
      ...
      console.log(`  Coverage: ${gameLogTypes.size > 0 ? Math.round((overlapping.size / Math.max(gameLogTypes.size, propsTypes.size)) * 100) : 0}%`);
    ...
    if (!finalGlDateErr && !finalPrDateErr) {
      ...
      console.log(`  Coverage: ${Math.max(gameLogDates.size, propsDates.size) > 0 ? Math.round((overlappingDates.size / Math.max(gameLogDates.size, propsDates.size)) * 100) : 0}%`);
