...
export default function PlayerPropsAnalyticsEnhanced({ league }: { league: string }) {
  ...
  const [sortBy, setSortBy] = useState<
    "player" | "market" | "line" | "matchup_rank" | "last5_avg" | "last10_avg" | "last20_avg"
  >("player");
  ...
  const getAnalyticsForPlayer = (playerName: string, propType: string) => {
    const matchupRank = analyticsData.matchupRank.find(
      (item) => item.player_name === playerName && item.prop_type === propType,
    );
    const last5 = analyticsData.last5.find(
      (item) => item.player_name === playerName && item.prop_type === propType,
    );
    const last10 = analyticsData.last10.find(
      (item) => item.player_name === playerName && item.prop_type === propType,
    );
    const last20 = analyticsData.last20.find(
      (item) => item.player_name === playerName && item.prop_type === propType,
    );
    ...
    <div className="player-props-analytics-enhanced">
      ...
      {showAnalytics && (
        <div className="analytics-status">
          ...
          {analyticsData.error && (
            <div className="error">Analytics Error: {analyticsData.error}</div>
          )}
          {!analyticsData.loading && !analyticsData.error && (
            <div className="analytics-summary">
              Analytics loaded: {analyticsData.matchupRank.length} matchup rankings,{" "}
              {analyticsData.last5.length} last-5, {analyticsData.last10.length} last-10,{" "}
              {analyticsData.last20.length} last-20
            </div>
          )}
        </div>
      )}
      ...
      {filteredEvents.map((event) => (
        <div key={event.eventID} className="event-block">
          ...
          <p className="event-stats">
            {event.player_props.length} props • {getUniqueMarketTypes(event.player_props).length}{" "}
            markets
          </p>
                  ...
                  {event.player_props.map((prop, idx) => {
                    ...
                    return (
                      <tr key={idx} className="prop-row">
                        ...
                        <td className="odds over">{formatOdds(prop.best_over)}</td>
                        <td className="odds under">{formatOdds(prop.best_under)}</td>
                        {showAnalytics && (
                          <>
                              ...
                              {analytics?.matchupRank ? (
                                <div className="analytics-cell">
                                  <div className="hit-rate">
                                    ...
                                    {analytics.matchupRank.margin
                                      ? ` (${analytics.matchupRank.margin.toFixed(1)})`
                                      : ""}
                                  </div>
                            ...
                            <td className="analytics last5">
                              {analytics?.last5 ? (
                                <div className="analytics-cell">
                                  <div className="avg-value">{analytics.last5.avg_value}</div>
                                  <div className="analytics-detail">
                            ...
                            <td className="analytics last10">
                              {analytics?.last10 ? (
                                <div className="analytics-cell">
                                  <div className="avg-value">{analytics.last10.avg_value}</div>
                                  <div className="analytics-detail">
                                    {analytics.last10.total_games} games •{" "}
                                    {analytics.last10.improvement > 0 ? "+" : ""}
                                    {analytics.last10.improvement}
                                  </div>
                                </div>
                              ) : (
                                <span className="no-data">-</span>
                            ...
                            <td className="analytics last20">
                              {analytics?.last20 ? (
                                <div className="analytics-cell">
                                  <div className="avg-value">{analytics.last20.avg_value}</div>
                                  <div className="analytics-detail">
                                    {analytics.last20.total_games} games • σ
                                    {analytics.last20.consistency}
                                  </div>
                                </div>
                              ) : (
                                <span className="no-data">-</span>
