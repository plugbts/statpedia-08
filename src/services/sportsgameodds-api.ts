...
// IMPORTANT: Do NOT embed secrets in client code. This value is only for non-sensitive contexts.
// Live requests should go through the server (/api/props). Frontend key remains empty by default.
const SPORTSGAMEODDS_API_KEY: string =
  (typeof import.meta !== "undefined" && (import.meta as any)?.env?.VITE_SPORTSGAMEODDS_API_KEY) ||
  "";
...
const API_KEY_INFO = {
  ...
  keyPrefix: SPORTSGAMEODDS_API_KEY ? SPORTSGAMEODDS_API_KEY.substring(0, 8) : "",
  keySuffix: SPORTSGAMEODDS_API_KEY ? SPORTSGAMEODDS_API_KEY.slice(-4) : "",
...
class SportsGameOddsAPI {
  ...
  private playerPropsCache = new Map<
    string,
    {
  ...
  private loadUsageStatsFromStorage() {
    try {
      ...
      if (stored) {
        ...
        logInfo(
          "SportsGameOddsAPI",
          `Loaded persistent usage stats: ${parsed.totalCalls} total calls, ${parsed.callsToday} today`,
        );
        return parsed;
      }
    } catch (error) {
  ...
  private trackAPIUsage(endpoint: string): void {
    ...
    if (usagePercentage >= 80) {
      logWarning(
        "SportsGameOddsAPI",
        `High API usage: ${this.usageStats.callsToday}/${this.MAX_DAILY_CALLS} calls today (${usagePercentage.toFixed(1)}%)`,
      );
    }
  }
  ...
  private handleRateLimit(): void {
    ...
    const backoffTime = Math.min(
      this.BASE_BACKOFF_MS * this.backoffMultiplier,
      this.MAX_BACKOFF_MS,
    );
    ...
    logWarning(
      "SportsGameOddsAPI",
      `Rate limited! Backing off for ${Math.ceil(backoffTime / 1000)} seconds (multiplier: ${this.backoffMultiplier})`,
    );
  }
    ...
    try {
      ...
      if (rateLimitLimit) {
        ...
        if (!isNaN(dailyLimit) && dailyLimit !== API_KEY_INFO.estimatedDailyLimit) {
          logInfo(
            "SportsGameOddsAPI",
            `Detected actual daily limit from headers: ${dailyLimit} (configured: ${API_KEY_INFO.estimatedDailyLimit})`,
          );
  ...
  getPlayerPropsCacheStatus(): string {
    ...
    const now = Date.now();
    const validCaches = Array.from(this.playerPropsCache.entries()).filter(
      ([_, cache]) => now - cache.timestamp < this.CACHE_DURATION.PLAYER_PROPS,
    );
  ...
  private async makeRequest<T>(
    endpoint: string,
    cacheDuration: number = CACHE_DURATION.ODDS,
  ): Promise<T> {
    ...
    if (this.shouldAvoidAPICall()) {
      ...
      if (this.cache.has(cacheKey)) {
        ...
        logWarning(
          "SportsGameOddsAPI",
          `Using expired cached data due to rate limit for ${endpoint}`,
        );
        return cached.data;
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          logWarning(
            "SportsGameOddsAPI",
            `Rate limit exceeded for ${endpoint}. Status: ${response.status}`,
          );
          ...
          if (this.cache.has(cacheKey)) {
            ...
            logWarning(
              "SportsGameOddsAPI",
              `Returning cached data due to rate limit for ${endpoint}`,
            );
            return cached.data;
          }
      ...
      if (data.success === false && data.error && data.error.includes("Rate limit")) {
        ...
        logWarning(
          "SportsGameOddsAPI",
          `Rate limit exceeded in response for ${endpoint}: ${data.error}`,
        );
  ...
  async getSports(): Promise<SportsGameOddsSport[]> {
    try {
      ...
      const sports: SportsGameOddsSport[] =
  ...
  async getBookmakers(): Promise<SportsGameOddsBookmaker[]> {
    try {
      ...
      const bookmakers: SportsGameOddsBookmaker[] =
        data.bookmakers?.map((bookmaker: any) => ({
          id: bookmaker.id,
          name: bookmaker.name,
  ...
  async getPlayerProps(sport: string): Promise<SportsGameOddsPlayerProp[]> {
    try {
      ...
      if (cached) {
        ...
        logAPI(
          "SportsGameOddsAPI",
          `Cache duration: ${Math.round(this.CACHE_DURATION.PLAYER_PROPS / 1000)}s`,
        );
        ...
        if (!isExpired) {
          ...
          console.log(
            `📊 Cached props being returned:`,
            cachedTestingProps.map(
              (p) =>
                `${p.playerName} ${p.propType} ${p.line} (${p.availableSportsbooks?.length || 0} books)`,
            ),
          );
          ...
          logAPI(
            "SportsGameOddsAPI",
            `⚡ EXACT DATA (CACHED): Using cached props for ${sport}, returning ${cachedTestingProps.length} of ${cached.props.length} for testing`,
          );
          this.usageStats.cacheHits++;
      ...
      logAPI(
        "SportsGameOddsAPI",
        `Fetching fresh player props for ${sport} from SportsGameOdds markets`,
      );
      ...
      logAPI(
        "SportsGameOddsAPI",
        `Skipping markets endpoint (doesn't exist) - using events endpoint only`,
      );
      ...
      do {
        try {
          ...
          if (response.data && Array.isArray(response.data)) {
            ...
            logAPI(
              "SportsGameOddsAPI",
              `Fetched ${response.data.length} events for ${sport} (total: ${allEvents.length})`,
            );
            logAPI("SportsGameOddsAPI", `First event structure:`, response.data[0]);
          } else {
            logWarning(
              "SportsGameOddsAPI",
              `Invalid response format from events endpoint:`,
              response,
            );
          ...
          if (error.message && error.message.includes("Rate limit")) {
            logWarning(
              "SportsGameOddsAPI",
              `Rate limit exceeded for ${sport}. Using cached data if available.`,
            );
            ...
            if (this.cache.has(cacheKey)) {
              ...
              logError(
                "SportsGameOddsAPI",
                `No cached data available for ${sport} and rate limit exceeded`,
              );
      ...
      for (const event of allEvents) {
        try {
          ...
          logAPI(
            "SportsGameOddsAPI",
            `Event ${event.eventID} produced ${eventPlayerProps.length} player props`,
          );
          ...
          if (eventPlayerProps.length === 0) {
            logAPI(
              "SportsGameOddsAPI",
              `No player props found in event odds for event ${event.eventID}`,
            );
          }
        } catch (error) {
          logWarning(
            "SportsGameOddsAPI",
            `Failed to extract props from event ${event.eventID}:`,
            error,
          );
        }
      }
      ...
      if (playerProps.length === 0) {
        logWarning(
          "SportsGameOddsAPI",
          `No player props found in SportsGameOdds markets for ${sport}. This could mean no games are currently available or no player prop markets exist for the current time period.`,
        );
      } else {
        logSuccess(
          "SportsGameOddsAPI",
          `Successfully extracted ${playerProps.length} player props using SportsGameOdds v2 consensus odds system for ${sport}`,
        );
      }
      ...
      logSuccess(
        "SportsGameOddsAPI",
        `⚡ EXACT DATA: Retrieved ${originalCount} consolidated props, returning ${testingProps.length} for testing`,
      );
      logAPI(
        "SportsGameOddsAPI",
        `Testing props: ${testingProps.map((p) => `${p.playerName} ${p.propType} (${p.availableSportsbooks?.length || 0} sportsbooks)`).join(", ")}`,
      );
  ...
  private async extractPlayerPropsFromEvent(
    ...
    logAPI(
      "SportsGameOddsAPI",
      `Extracting player props from event ${event.eventID} using v2 byBookmaker structure`,
    );
    ...
    logAPI(
      "SportsGameOddsAPI",
      `Processing ${Object.keys(event.odds).length} odds for event ${event.eventID}`,
    );
    ...
    for (const [oddId, oddData] of Object.entries(event.odds)) {
      try {
        ...
        if (this.isPlayerPropMarket(odd, oddId)) {
          ...
          const bookmakerProps = this.extractPlayerPropsFromBookmakers(
            odd,
            oddId,
            sport,
            homeTeam,
          ...
          if (bookmakerProps.length > 0) {
            ...
            logAPI(
              "SportsGameOddsAPI",
              `Added ${bookmakerProps.length} bookmaker props for ${oddId}`,
            );
          } else {
            logAPI(
              "SportsGameOddsAPI",
              `No bookmaker props found for ${oddId} - skipping consensus`,
            );
          }
        }
      } catch (error) {
    ...
    logAPI(
      "SportsGameOddsAPI",
      `Extracted ${playerProps.length} player props from event ${event.eventID} using v2 structure`,
    );
    return playerProps;
  }

  // Extract player props from each bookmaker's odds data using v2 byBookmaker structure
  private extractPlayerPropsFromBookmakers(
    odd: any,
    oddId: string,
    sport: string,
    homeTeam: string,
    ...
    if (!odd.byBookmaker) {
      logAPI(
        "SportsGameOddsAPI",
        `No byBookmaker data for oddID ${oddId}, skipping consensus odds`,
      );
    ...
    for (const [bookmakerId, overBookmakerOdds] of Object.entries(odd.byBookmaker)) {
      try {
        ...
        console.log(
          `⚠️  Same Odds Issue: ${overData.odds === underData.odds ? "YES - PROBLEM!" : "NO - Good"}`,
        );
        console.log("=====================================");
        ...
        logWarning(
          "SportsGameOddsAPI",
          `Failed to process bookmaker ${bookmakerId} for odd ${oddId}:`,
          error,
        );
      }
    }
  ...
  private createBookmakerPlayerProp(
    ...
    try {
      ...
      if (overOdds === null || underOdds === null) {
        logWarning(
          "SportsGameOddsAPI",
          `Missing odds data for ${bookmakerId} ${playerName} ${propType}: over=${rawOverOdds}, under=${rawUnderOdds}`,
        );
        return null;
      }
      ...
      logAPI(
        "SportsGameOddsAPI",
        `EXACT ${this.mapBookmakerIdToName(bookmakerId)} odds: ${playerName} ${propType} ${line} - Over: ${overOdds} Under: ${underOdds}`,
      );
      ...
      if (isNaN(line) || isNaN(overOdds) || isNaN(underOdds)) {
        logWarning(
          "SportsGameOddsAPI",
          `Invalid numeric data for ${playerName}: line=${line}, overOdds=${overOdds}, underOdds=${underOdds}`,
        );
        return null;
      }
  ...
  private groupPropsBySportsbookAvailability(
    ...
    for (const [groupKey, group] of propGroups) {
      ...
      console.log(
        `📊 CONSOLIDATED PROP: ${baseProp.playerName} ${baseProp.propType} ${baseProp.line}`,
      );
  ...
  private convertPlayerToPlayerProp(
    player: any,
    sport: string,
    eventId: string,
  ): SportsGameOddsPlayerProp | null {
  ...
  private extractTeamFromPlayerID(
    playerID: string,
    homeTeam: string,
    awayTeam: string,
    event?: any,
  ...
  private isPlayerPropMarket(market: any, oddId: string): boolean {
    ...
    const isSimplePlayerID =
      statEntityID &&
      statEntityID !== "0" &&
      statEntityID !== "null" &&
      statEntityID !== "undefined" &&
      statEntityID.length > 3;
    ...
    logAPI(
      "SportsGameOddsAPI",
      `Checking oddID: ${oddId} - isPlayerID: ${isPlayerID}, isSimplePlayerID: ${isSimplePlayerID}, isPlayerStat: ${isPlayerStat}, isOverUnder: ${isOverUnder}, statEntityID: ${statEntityID}`,
    );
  ...
  async getGames(sport: string): Promise<SportsGameOddsGame[]> {
    try {
      ...
      const games: SportsGameOddsGame[] =
