...
class HasuraPlayerPropsAPI {
  ...
  /**
   * Get Pick'em props from separate table
   */
  async getPickemProps(sport?: string): Promise<PlayerProp[]> {
    try {
      const startTime = Date.now();
      
      // Build GraphQL query for pickem props
      const sportFilter = sport ? `, where: { player: { team: { league: { code: { _eq: "${sport.toUpperCase()}" } } } } }` : '';
      const query = `
        query GetPickemProps($limit: Int) {
          pickem_props(limit: $limit, order_by: {updated_at: desc}${sportFilter}) {
            id
            prop_type
            line
            pickem_site
      ...
      const result = await response.json();
      const pickemProps = result.data?.pickem_props || [];

      // Transform pickem props to match PlayerProp interface
      const transformedProps: PlayerProp[] = pickemProps.map((prop: any) => {
        const league = prop.player?.team?.league;
        return {
          id: prop.id,
          player: prop.player?.name || 'Unknown Player',
          playerId: prop.player?.id,
          team: prop.player?.team?.name || 'Unknown Team',
          teamAbbr: prop.player?.team?.abbreviation || 'UNK',
          position: prop.player?.position || '',
          opponent: 'OPP', // TODO: Add opponent relationship
          opponentAbbr: 'OPP',
          gameId: prop.game_id,
          sport: league?.code || sport?.toUpperCase() || 'UNKNOWN',
          propType: prop.prop_type || 'Unknown',
          line: prop.line ? parseFloat(prop.line) : null,
          source: 'pickem',
          overOdds: prop.over_projection ? parseInt(prop.over_projection.toString()) : null,
          underOdds: prop.under_projection ? parseInt(prop.under_projection.toString()) : null,
          pickemSite: prop.pickem_site,
          overProjection: prop.over_projection ? parseFloat(prop.over_projection) : null,
          underProjection: prop.under_projection ? parseFloat(prop.under_projection) : null,
          gameDate: new Date().toISOString().split('T')[0],
          gameTime: new Date().toISOString(),
          availableSportsbooks: [prop.pickem_site],
          allSportsbookOdds: [{
            sportsbook: prop.pickem_site,
            overOdds: prop.over_projection ? parseInt(prop.over_projection.toString()) : null,
            underOdds: prop.under_projection ? parseInt(prop.under_projection.toString()) : null,
          }],
          lastUpdate: new Date().toISOString(),
          priority: false, // Pickem props are not priority
        };
      });

      // Sort pickem props by prop type priority
      const sortedProps = transformedProps.sort((a, b) => {
        const propPriority = (propType: string) => {
          const lower = propType.toLowerCase();
          if (lower.includes('passing yards')) return 1;
          if (lower.includes('rushing yards')) return 2;
      ...
      const responseTime = Date.now() - startTime;
      console.log(`✅ HASURA: Successfully retrieved ${sortedProps.length} pickem props (${responseTime}ms)`);

      return sortedProps;
    } catch (error) {
      console.error('❌ HASURA: Failed to fetch pickem props:', error);
      return [];
    }
  }
}
