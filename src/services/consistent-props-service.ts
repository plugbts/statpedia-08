...
class ConsistentPropsService {
  ...
  private calculateConfidence(factors: ConfidenceFactor[]): number {
    ...
    factors.forEach((factor) => {
      const impactMultiplier =
        factor.impact === "positive" ? 1 : factor.impact === "negative" ? -1 : 0;
    ...
    const normalizedScore =
      totalWeight > 0 ? Math.max(0, Math.min(1, (weightedScore / totalWeight + 1) / 2)) : 0.5;
  ...
  async getOddsForLine(
    prop: ConsistentPlayerProp,
    newLine: number,
  ): Promise<SportsbookOdds | null> {
    try {
      logAPI(
        "ConsistentPropsService",
        `Getting real sportsbook odds for line ${newLine} from SportGameOdds API`,
      );
  ...
  private async fetchRealAlternativeLineOdds(
    prop: ConsistentPlayerProp,
    newLine: number,
  ): Promise<SportsbookOdds | null> {
    try {
      ...
      logAPI(
        "ConsistentPropsService",
        `Fetching alternative line ${newLine} for ${prop.playerName} ${prop.propType}`,
      );
      ...
      if (matchingProps.length > 0) {
        ...
        logSuccess(
          "ConsistentPropsService",
          `Found real sportsbook odds for line ${newLine}: Over ${matchingProp.overOdds}, Under ${matchingProp.underOdds}`,
        );
      ...
      logAPI(
        "ConsistentPropsService",
        `No exact line match found for ${newLine}, will try interpolation`,
      );
      return null;
    } catch (error) {
  ...
  private async interpolateFromNearbyLines(
    prop: ConsistentPlayerProp,
    newLine: number,
  ): Promise<SportsbookOdds | null> {
    try {
      ...
      const samePlayerProps = allProps
        .filter(
          (p) =>
            p.playerName === prop.playerName &&
            p.propType === prop.propType &&
      ...
      if (samePlayerProps.length < 2) {
        logWarning(
          "ConsistentPropsService",
          `Not enough data points for interpolation, using fallback calculation`,
        );
      ...
      if (lowerLine && upperLine && lowerLine.line !== upperLine.line) {
        ...
        logSuccess(
          "ConsistentPropsService",
          `Interpolated odds for line ${newLine}: Over ${interpolatedOdds.over}, Under ${interpolatedOdds.under}`,
        );
      ...
      if (referenceLine) {
        const extrapolatedOdds = this.extrapolateOdds(
          referenceLine,
          newLine,
          prop.propType,
          prop.sport,
        ...
        logAPI(
          "ConsistentPropsService",
          `Extrapolated odds for line ${newLine}: Over ${extrapolatedOdds.over}, Under ${extrapolatedOdds.under}`,
        );
  ...
  private getProbabilityShiftPerLine(propType: string, sport: string, line: number): number {
    ...
    if (sportLower === "nfl") {
      ...
      if (propTypeLower.includes("receiving yards")) return 0.1; // 10% per yard
      ...
      if (propTypeLower.includes("points")) return 0.1; // 10% per point
      if (propTypeLower.includes("rebounds")) return 0.18; // 18% per rebound
      if (propTypeLower.includes("assists")) return 0.2; // 20% per assist
      ...
      if (propTypeLower.includes("hits")) return 0.2; // 20% per hit
   ...
   */
  async getConsistentPlayerProps(
    sport: string,
    selectedSportsbook?: string,
  ): Promise<ConsistentPlayerProp[]> {
    ...
    if (cached && Date.now() - cached.timestamp < this.CACHE_DURATION) {
      logAPI(
        "ConsistentPropsService",
        `Using cached props for ${sport} (${cached.props.length} props)`,
      );
      return cached.props;
    }
    ...
    try {
      ...
      logAPI(
        "ConsistentPropsService",
        `Retrieved ${baseProps.length} exact sportsbook props from backend API`,
      );
      console.log(
        "🎯 ConsistentPropsService received exact sportsbook props from backend:",
        baseProps,
      );
      ...
      logSuccess(
        "ConsistentPropsService",
        `Generated ${sortedProps.length} consistent props for ${sport}`,
      );
    ...
    for (const prop of existingProps) {
      try {
        ...
        if (fanduelOdds) {
          ...
          const fanduelIndex = prop.allSportsbookOdds.findIndex(
            (odds) => odds.sportsbook === "FanDuel",
          );
          if (fanduelIndex >= 0) {
  ...
  private calculateBasicEV(overOdds: number, underOdds: number): number {
    ...
    const overEV =
      trueProbability * (overOdds > 0 ? overOdds / 100 : 100 / Math.abs(overOdds)) -
      (1 - trueProbability) * 1;
