// ARCHIVE COPY - 2025-10-19
// This is a preserved snapshot of the previous analytics PropsTable.
// Keep for potential rollback or reference.
  ...
  if (rows.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500">
        <p>
          No props available for {LEAGUE_DISPLAY_NAMES[league] || league.toUpperCase()} on this
          date.
        </p>
      </div>
    );
          ...
          {rows.map((row) => (
            <tr key={row.prop_id} className="hover:bg-gray-50/50">
              <td className="px-4 py-3 whitespace-nowrap">
                ...
                <div className="text-xs text-gray-500">{row.game_id}</div>
              </td>
              <td className="px-4 py-3 whitespace-nowrap">
                <div className="text-sm text-gray-900 capitalize">{row.prop_type}</div>
              </td>
...
// GameMatchupTable archive as well for completeness
...
export function GameMatchupTable({ games, league, className = "" }: GameMatchupTableProps) {
  if (games.length === 0) {
    return (
      <div className="text-center py-8 text-gray-500">
        <p>
          No games available for {LEAGUE_DISPLAY_NAMES[league] || league.toUpperCase()} on this
          date.
        </p>
      </div>
    );
          ...
          {games.map((game, index) => (
            <tr key={`${game.game_id}-${game.prop_type}-${index}`} className="hover:bg-gray-50">
              <td className="px-6 py-4 whitespace-nowrap">
                ...
                <div className="text-sm text-gray-500 capitalize">{game.prop_type}</div>
              </td>
