...
export const AIPoweredInsightsTab: React.FC<AIPoweredInsightsTabProps> = ({ 
  ...
  const [predictionAnalytics, setPredictionAnalytics] = useState<AIPoweredPredictionAnalytics | null>(null);
  ...
  useEffect(() => {
    const loadInsights = async () => {
      ...
      try {
        console.log(`🧠 [AIPoweredInsightsTab] Loading AI-powered insights for ${selectedSport}...`);
        ...
        console.log(`✅ [AIPoweredInsightsTab] Successfully loaded AI-powered insights for ${selectedSport}`);
      } catch (error) {
  ...
  const refreshInsights = async () => {
    ...
    try {
      console.log(`🧠 [AIPoweredInsightsTab] Refreshing AI-powered insights for ${selectedSport}...`);
      ...
      console.log(`✅ [AIPoweredInsightsTab] Successfully refreshed AI-powered insights for ${selectedSport}`);
    } catch (error) {
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{(insight.ai_analysis.model_consensus * 100).toFixed(1)}% AI agreement</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{insight.ai_analysis.expected_value.toFixed(1)}% EV</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{insight.ai_analysis.risk_score} risk factors</p>
              </div>
            </div>
          </div>
          <div className="space-y-4">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  {insight.ai_analysis.key_factors.map((factor, idx) => (
                    <span key={idx}>
                      {factor}{idx < insight.ai_analysis.key_factors.length - 1 ? ', ' : ''}
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  {insight.ai_analysis.data_sources.map((source, idx) => (
                    <span key={idx}>
                      {source}{idx < insight.ai_analysis.data_sources.length - 1 ? ', ' : ''}
                    </span>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{(insight.ai_analysis.advanced_model_score * 100).toFixed(1)}% confidence</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-2 h-2 rounded-full bg-blue-500 mt-2"></div>
              <div>
                ...
                <p className="text-sm text-muted-foreground">{(insight.ai_analysis.ml_model_score * 100).toFixed(1)}% confidence</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-2 h-2 rounded-full bg-green-500 mt-2"></div>
              <div>
                ...
                <p className="text-sm text-muted-foreground">{(insight.ai_analysis.ensemble_score * 100).toFixed(1)}% consensus</p>
              </div>
            </div>
          </div>
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{insight.ai_analysis.data_quality}/100 quality score</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  {insight.ai_analysis.risk_factors.map((risk, idx) => (
                    <span key={idx}>
                      {risk}{idx < insight.ai_analysis.risk_factors.length - 1 ? ', ' : ''}
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  {insight.ai_analysis.key_insights.map((insight, idx) => (
                    <span key={idx}>
                      {insight}{idx < insight.ai_analysis.key_insights.length - 1 ? ', ' : ''}
                    </span>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{(insight.ai_analysis.situational_motivation * 100).toFixed(1)}% motivation score</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-2 h-2 rounded-full bg-green-500 mt-2"></div>
              <div>
                ...
                <p className="text-sm text-muted-foreground">{(insight.ai_analysis.matchup_advantage * 100).toFixed(1)}% advantage</p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              <div className="w-2 h-2 rounded-full bg-blue-500 mt-2"></div>
              <div>
                ...
                <p className="text-sm text-muted-foreground">{(insight.ai_analysis.weather_impact * 100).toFixed(1)}% impact factor</p>
              </div>
            </div>
          </div>
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground space-y-1">
                  <div>EPA Differential: {insight.ai_analysis.advanced_metrics.epa_differential.toFixed(2)}</div>
                  <div>Success Rate: {(insight.ai_analysis.advanced_metrics.success_rate_differential * 100).toFixed(1)}%</div>
                  <div>Pace Advantage: {(insight.ai_analysis.advanced_metrics.pace_advantage * 100).toFixed(1)}%</div>
                </div>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">
                  {insight.ai_analysis.psychological_factors.map((factor, idx) => (
                    <span key={idx}>
                      {factor}{idx < insight.ai_analysis.psychological_factors.length - 1 ? ', ' : ''}
                    </span>
                  ))}
                </div>
              </div>
    ...
    return (
      <div className="space-y-8">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-2">Error Loading AI-Powered Insights</h3>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex-1 text-center">
            ...
            <p className="text-muted-foreground">
              Advanced AI analysis powered by machine learning models and SportsGameOdds API for {selectedSport.toUpperCase()}
            </p>
          </div>
      ...
      <Tabs value={activeFilter} onValueChange={(value) => setActiveFilter(value as 'all' | 'game' | 'player' | 'moneyline' | 'underdogs' | 'regular-moneyline')}>
        ...
        <TabsContent value="underdogs" className="space-y-6">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold text-foreground">AI-Powered Underdog Opportunities</h2>
            ...
            <p className="text-muted-foreground">
              Advanced AI analysis identifies the best underdog moneyline opportunities using machine learning models and comprehensive data analysis
            </p>
            
            <div className="grid grid-cols-1 gap-6">
              {moneylineInsights.filter(insight => insight.underdog_opportunity).map(renderAIPoweredMoneylineInsight)}
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="regular-moneyline" className="space-y-6">
          <div className="space-y-4">
            ...
            <p className="text-muted-foreground">
              Comprehensive AI-powered moneyline analysis with advanced team performance metrics and sophisticated matchup insights
            </p>
