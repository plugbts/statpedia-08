...
const SeasonalParticle: React.FC<{
  ...
  return (
    <div
      className={cn("absolute pointer-events-none text-white/80", getAnimationClass())}
      style={{
        left: `${left}%`,
...
export const SeasonalVideoBackground: React.FC<SeasonalVideoBackgroundProps> = ({
  ...
  const [currentSeason, setCurrentSeason] = useState<"spring" | "summer" | "fall" | "winter">(
    "winter",
  );
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-xl bg-gradient-card border border-border/50 min-h-[400px]",
        className,
      )}
    >
      {/* Seasonal background */}
