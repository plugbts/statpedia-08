...
export const ParlayGen: React.FC = () => {
  ...
  useEffect(() => {
    if (!hasPremiumAccess) {
      ...
      toast({
        ...
        description: "Upgrade to Premium to use Parlay Generator.",
      });
    }
  ...
  const loadRealData = async () => {
    try {
      ...
      const gamesData = await gamesService.getCurrentWeekPredictions(
        sportFilter === "all" ? "nfl" : sportFilter.toLowerCase(),
      );
  ...
  const generateParlay = async () => {
    ...
    try {
      ...
      newParlay.totalConfidence =
        newParlay.legs.reduce((acc, leg) => acc + leg.confidence, 0) / newParlay.legs.length;
  ...
  const generateParlayLegsFromRealData = (): ParlayLeg[] => {
    ...
    if (sportFilter !== "all") {
      filteredProps = availableProps.filter(
        (prop) => prop.game_date && new Date(prop.game_date) >= new Date(), // Only future games
      );
    }
    ...
    if (filteredProps.length > 0) {
      ...
      const selectedProps = filteredProps.sort(() => Math.random() - 0.5).slice(0, legCount);

      selectedProps.forEach((prop, index) => {
        const game = availableGames.find(
          (g) => g.game.homeTeam === prop.team || g.game.awayTeam === prop.team,
        );
      ...
      const selectedGames = availableGames.sort(() => Math.random() - 0.5).slice(0, legCount);

      selectedGames.forEach((gamePrediction, index) => {
        const game = gamePrediction.game;
        const odds =
          gamePrediction.prediction.homeWinProbability > 0.5
        ...
        legs.push({
          ...
          player:
            gamePrediction.prediction.homeWinProbability > 0.5 ? game.homeTeam : game.awayTeam,
          prop: "Moneyline",
          line: "Win",
          odds: odds,
          prediction: "over",
  ...
  const formatNumber = (
    value: number,
    type: "odds" | "payout" | "value" | "percentage",
  ): string => {
    if (type === "odds") {
  ...
  const generateRealReasoning = (
    prop: PlayerPropPrediction,
    game: GamePrediction,
    confidence: number,
  ): string => {
  ...
  const sendToSportsbook = async (parlay: GeneratedParlay) => {
    ...
    toast({
      ...
      description:
        "This feature will send your parlay to connected sportsbooks. Make sure you have sportsbooks connected in Bet Tracking.",
      variant: "default",
    });
  };

  // Always render the page; overlay when locked for consistent theme
    ...
    <div className="min-h-screen bg-background p-4 relative">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Customize your parlay parameters for optimal results</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setLegCount(parseInt(value))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={oddsFilter}
                  onValueChange={(value: "under" | "over") => setOddsFilter(value)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={oddsValue.toString()}
                  onValueChange={(value) => setOddsValue(parseInt(value))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            {isLoadingData && (
              <Alert>
                ...
                <AlertDescription>Loading current week games and predictions...</AlertDescription>
              </Alert>
            )}
          </CardContent>
        </Card>
                  ...
                  <CardContent className="space-y-3">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          Confidence: {formatNumber(parlay.totalConfidence, "percentage")}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
        ...
        {selectedParlay && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Detailed breakdown of your selected parlay</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-muted/50 rounded-lg">
                <div className="text-center">
                  <div className="text-2xl font-bold text-primary">
                    {formatNumber(selectedParlay.totalOdds, "odds")}
                  </div>
                  <div className="text-sm text-muted-foreground">Total Odds</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-500">
                    {formatNumber(selectedParlay.totalConfidence, "percentage")}
                  </div>
                  <div className="text-sm text-muted-foreground">Confidence</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-purple-500">
                    {formatNumber(selectedParlay.expectedValue, "value")}
                  </div>
                  <div className="text-sm text-muted-foreground">Expected Value</div>
                </div>
                <div className="text-center">
                  <div className="text-2xl font-bold text-yellow-500">
                    ${formatNumber(selectedParlay.potentialPayout, "payout")}
                  </div>
                  ...
                  <Card key={leg.id} className="p-4">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-muted-foreground">{leg.reasoning}</div>
                      </div>
      ...
      {!hasPremiumAccess && (
        <SubscriptionOverlay
          ...
          description="Upgrade to Premium to generate AI-built parlays with EV and confidence."
