...
import { displayPropType, displayPropTypeForLeague } from "./prop-display-map";

export const formatPropType = (propType: string, league?: string): string => {
  if (!propType) return "Unknown Prop";

  // Use the clean display mapping first (prevents concatenation bugs)
  const cleanDisplay = league
    ? displayPropTypeForLeague(league, propType)
    : displayPropType(propType);
