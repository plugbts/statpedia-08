-- Analytics Schema Migration for PropFinder-style Analytics
...
CREATE TABLE IF NOT EXISTS player_game_logs (
  ...
  UNIQUE(player_id, game_id, prop_type, line),
  INDEX idx_player_game_logs_player_date (player_id, game_date),
  INDEX idx_player_game_logs_prop_type (prop_type),
  INDEX idx_player_game_logs_opponent (opponent_id)
);
...
CREATE TABLE IF NOT EXISTS defense_ranks (
  ...
  INDEX idx_defense_ranks_team (team_id),
  INDEX idx_defense_ranks_prop_type (prop_type),
  INDEX idx_defense_ranks_league (league_id)
);

-- 4. Create function to calculate hit rates and streaks
...
BEGIN
  ...
  LOOP
    ...
    ELSIF array_length(hits, 1) > 1 THEN
      IF logs.hit = (SELECT hit FROM player_game_logs WHERE player_id = p_player_id AND prop_type = p_prop_type AND line = p_line ORDER BY game_date DESC LIMIT 1 OFFSET array_length(hits, 1) - 2) THEN
...
-- 5. Create function to calculate H2H hit rate vs opponent
...
-- 6. Create function to get matchup grade from defensive rank
...
-- 7. Create comprehensive analytics update function
...
-- 8. Create trigger to automatically update analytics when props are inserted/updated
CREATE OR REPLACE FUNCTION trigger_update_prop_analytics() RETURNS TRIGGER AS $$
BEGIN
  PERFORM update_prop_analytics(NEW.id);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER update_prop_analytics_trigger
  AFTER INSERT OR UPDATE ON props
  FOR EACH ROW
  EXECUTE FUNCTION trigger_update_prop_analytics();
