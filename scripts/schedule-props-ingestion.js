...
if (!DATABASE_URL || !API_KEY) {
  console.error(
    "❌ Missing required environment variables: DATABASE_URL and SPORTSGAMEODDS_API_KEY",
  );
  process.exit(1);
}
...
function runIngestion(leagues = "ALL", label = "Ingestion") {
  ...
  try {
    const cmd = `npm run ingest:props:drizzle ${leagues}`;
    const output = execSync(cmd, {
      ...
      env: {
        ...
        SPORTSGAMEODDS_API_KEY: API_KEY,
      },
    });
    console.log(output);
...
function validateIngestion() {
  try {
    const cmd = `node scripts/validate-props.mjs`;
    const output = execSync(cmd, {
      encoding: "utf8",
      stdio: "pipe",
      env: {
...
// Note: node-cron doesn't support hour ranges that wrap across midnight, so we enumerate hours
cron.schedule(
  "*/5 23,0,1,2,3,4 * * 1-6",
  () => {
