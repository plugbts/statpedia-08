...
function normalizePropType(propType: string): string {
  switch (propType.toLowerCase().replace(/\s+/g, '_')) {
    // NFL
    case "passing_yards": case "pass_yards": return "Passing Yards";
    case "passing_tds": case "pass_tds": return "Passing TDs";
    case "passing_completions": case "completions": return "Passing Completions";
    case "pass_attempts": return "Pass Attempts";
    case "rushing_yards": case "rush_yards": return "Rushing Yards";
    case "rushing_tds": case "rush_tds": return "Rushing TDs";
    case "rush_attempts": return "Rush Attempts";
    case "receiving_yards": case "rec_yards": return "Receiving Yards";
    case "receiving_tds": case "rec_tds": return "Receiving TDs";
    case "receptions": case "catches": return "Receptions";
    
    // NBA
    case "points": return "Points";
    case "assists": return "Assists";
    case "rebounds": return "Rebounds";
    case "3_pointers_made": case "three_pointers": return "3-Pointers Made";
    case "steals": return "Steals";
    case "blocks": return "Blocks";
    
    // MLB
    case "hits": return "Hits";
    case "strikeouts": case "strike_outs": return "Strikeouts";
    case "total_bases": case "tb": return "Total Bases";
    case "runs": return "Runs";
    case "rbis": case "rbi": return "RBIs";
    case "walks": case "bb": return "Walks";
    
    // NHL
    case "shots_on_goal": case "shots": return "Shots on Goal";
...
async function initializeMappings(league: League) {
  ...
  console.log(`Loaded ${teamMappings.length} team mappings and ${leaguePlayers.length} players for ${league}`);
}
...
async function fetchNBAGamesAndLogs(season: number = 2023, league: 'NBA' | 'WNBA' = 'NBA') {
  ...
  try {
    ...
    for (const game of games) {
      ...
      if (league === 'WNBA') {
        const wnbaTeams = ['CON', 'LVA', 'LAS', 'NYL', 'GSV', 'PHO', 'SEA', 'CHI', 'WAS', 'MIN', 'ATL', 'IND', 'DAL'];
        if (!wnbaTeams.includes(teamAbbr)) continue;
      }
    ...
    for (const stat of games) {
      if (stat.length > 20) { // Ensure we have enough data
    ...
    console.log(`Processed ${processedGames.length} ${league} games and ${processedLogs.length} player logs`);
...
async function fetchMLBGamesAndLogs(date: string) {
  ...
  try {
    // Fetch schedule
    const scheduleResp = await fetch(`https://statsapi.mlb.com/api/v1/schedule?sportId=1&date=${date}`);
    const scheduleData = await scheduleResp.json();
    ...
    for (const game of games) {
      ...
      const boxscoreResp = await fetch(`https://statsapi.mlb.com/api/v1/game/${game.gamePk}/boxscore`);
    ...
    console.log(`Processed ${processedGames.length} MLB games and ${processedLogs.length} player logs`);
...
async function fetchNHLGamesAndLogs(date: string) {
  ...
  try {
    ...
    for (const game of games) {
      ...
      const gameDetailResp = await fetch(`https://api-web.nhle.com/v1/gamecenter/${game.id}/boxscore`);
    ...
    console.log(`Processed ${processedGames.length} NHL games and ${processedLogs.length} player logs`);
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log('Usage: tsx real-data-ingestion-skeleton.ts [nfl|nba|wnba|mlb|nhl|all] [season|date]');
