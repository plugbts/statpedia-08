/**
 * Quick Prop Ingestion Script - Limited to avoid rate limits
...
async function ingestNFLProps() {
  console.log(`🎯 Processing NFL props for 2025...`);
  
  let totalProps = 0;
  
  try {
    // Fetch just 5 events to start
    const url = `https://api.sportsgameodds.com/v2/events?apiKey=${API_KEY}&oddsAvailable=true&leagueID=NFL&season=2025&limit=5`;
    
    console.log(`  📡 Fetching NFL events...`);
    
    const res = await fetch(url);
    if (!res.ok) {
      console.error(`❌ Failed to fetch props for NFL:`, res.status, await res.text());
      return 0;
    }
    ...
    for (const event of events) {
      // The props are in event.odds object
      const odds = event.odds || {};
      const players = event.players || {};
      
      // Extract player props from the odds object - only over/under props
      const playerProps = Object.entries(odds).filter(([key, odd]) => {
        return odd.playerID && odd.betTypeID === 'ou' && odd.sideID === 'over'; // Only over props to avoid duplicates
      });
      ...
      for (const [oddId, odd] of playerProps) {
        try {
          ...
          // Use a default team ID - in production you'd map this properly
  ...
  console.log(`✅ Completed NFL: ${totalProps} props ingested`);
  return totalProps;
}

async function main() {
  console.log(`🚀 Starting quick prop ingestion for 2025...`);
  console.log(`⏰ Started at: ${new Date().toISOString()}`);
  console.log('=' .repeat(60));

  const totalProps = await ingestNFLProps();
