...
async function investigateApiIngestion() {
  ...
  try {
    ...
    const gameLogsResponse = await fetch('https://api.sportsgameodds.com/events?league=nfl&limit=100', {
      headers: { 'x-api-key': API_KEY }
    });
    
    if (!gameLogsResponse.ok) {
      console.error(`❌ Game logs API error: ${gameLogsResponse.status} ${gameLogsResponse.statusText}`);
    } else {
      ...
      for (const event of gameLogsData.data || []) {
        if (event.results?.game) {
          const playerCount = Object.keys(event.results.game).filter(key => key !== 'away' && key !== 'home').length;
      ...
      Array.from(uniquePlayers).slice(0, 10).forEach((player, i) => {
    ...
    console.log('Testing: https://api.sportsgameodds.com/v2/events?apiKey=***&oddsAvailable=true&leagueID=NFL&season=2025&limit=100');
    const propLinesResponse = await fetch(`https://api.sportsgameodds.com/v2/events?apiKey=${API_KEY}&oddsAvailable=true&leagueID=NFL&season=2025&limit=100`);
    
    if (!propLinesResponse.ok) {
      console.error(`❌ Prop lines API error: ${propLinesResponse.status} ${propLinesResponse.statusText}`);
    } else {
      ...
      Array.from(uniquePropPlayers).slice(0, 10).forEach((player, i) => {
        console.log(`  ${i + 1}. ${player}`);
      });
    }
    ...
    if (gameLogsDatesError || propLinesDatesError) {
      ...
      const gameLogsDateRange = gameLogsDates?.length > 0 ? {
        earliest: gameLogsDates[gameLogsDates.length - 1].date,
        latest: gameLogsDates[0].date
      } : null;
      
      const propLinesDateRange = propLinesDates?.length > 0 ? {
    ...
    const gameLogsPage2Response = await fetch('https://api.sportsgameodds.com/events?league=nfl&limit=100&cursor=2', {
      headers: { 'x-api-key': API_KEY }
    });
