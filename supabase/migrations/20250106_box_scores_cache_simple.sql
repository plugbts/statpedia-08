...
CREATE POLICY IF NOT EXISTS "Allow read access to box scores" 
ON public.box_scores_cache 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

-- Allow service role to insert/update box scores (for scraping)
CREATE POLICY IF NOT EXISTS "Allow service role to manage box scores" 
ON public.box_scores_cache 
FOR ALL 
...
CREATE POLICY IF NOT EXISTS "Allow read access to defensive rankings" 
ON public.defensive_rankings_cache 
FOR SELECT 
USING (auth.uid() IS NOT NULL);

-- Allow service role to manage defensive rankings
CREATE POLICY IF NOT EXISTS "Allow service role to manage defensive rankings" 
ON public.defensive_rankings_cache 
FOR ALL 
USING (auth.role() = 'service_role');
