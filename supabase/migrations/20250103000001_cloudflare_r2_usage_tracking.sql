-- Cloudflare R2 Usage Tracking Migration
-- Creates tables for monitoring R2 storage usage and costs
...
USING (
  EXISTS (
    ...
    AND (subscription_status = 'admin' OR subscription_status = 'owner')
  )
);
...
-- Function to aggregate daily usage into summary table
CREATE OR REPLACE FUNCTION aggregate_daily_r2_usage()
RETURNS void AS $$
BEGIN
  INSERT INTO public.r2_usage_summary (
    bucket_name,
    usage_date,
    total_requests,
  ...
  SELECT 
    ...
    SUM(CASE WHEN operation_type = 'GET' THEN request_count ELSE 0 END) as get_requests,
    SUM(CASE WHEN operation_type = 'PUT' THEN request_count ELSE 0 END) as put_requests,
    SUM(CASE WHEN operation_type = 'DELETE' THEN request_count ELSE 0 END) as delete_requests,
    SUM(CASE WHEN operation_type = 'HEAD' THEN request_count ELSE 0 END) as head_requests,
    SUM(CASE WHEN operation_type = 'LIST' THEN request_count ELSE 0 END) as list_requests
  FROM public.r2_usage_logs
  WHERE date_trunc('day', created_at)::DATE = CURRENT_DATE - interval '1 day'
  GROUP BY bucket_name, date_trunc('day', created_at)::DATE
  ON CONFLICT (bucket_name, usage_date) DO UPDATE SET
    total_requests = EXCLUDED.total_requests,
    total_bytes_transferred = EXCLUDED.total_bytes_transferred,
    total_cost_usd = EXCLUDED.total_cost_usd,
    get_requests = EXCLUDED.get_requests,
    put_requests = EXCLUDED.put_requests,
    delete_requests = EXCLUDED.delete_requests,
    head_requests = EXCLUDED.head_requests,
    list_requests = EXCLUDED.list_requests,
    updated_at = now();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to reset monthly usage tracking
CREATE OR REPLACE FUNCTION reset_monthly_r2_usage()
RETURNS void AS $$
BEGIN
  -- Reset current usage for new month
  UPDATE public.r2_current_usage
  SET 
...
GRANT EXECUTE ON FUNCTION aggregate_daily_r2_usage() TO authenticated;
GRANT EXECUTE ON FUNCTION reset_monthly_r2_usage() TO authenticated;
