...
AS $function$
BEGIN
  IF NEW.role = 'moderator'::app_role AND NOT (has_role(auth.uid(), 'admin'::app_role) OR has_role(auth.uid(), 'owner'::app_role)) THEN
...
BEGIN
  SELECT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = NEW.user_id AND role = 'owner'::app_role) INTO target_has_owner_role;
  IF NEW.role = 'owner'::app_role AND NOT target_has_owner_role THEN
    RAISE EXCEPTION 'Owner role cannot be granted';
  END IF;
  IF TG_OP = 'UPDATE' AND OLD.role = 'owner'::app_role THEN
  ...
  END IF;
  IF NEW.role = 'admin'::app_role AND NOT public.is_owner(auth.uid()) THEN
