...
-- 5. Streaks: Compare gl.value vs p.line correctly
...
SELECT 
  ...
  -- Streaks: Calculate from player_game_logs (compare gl.value vs p.line)
  -- Get recent 5, 10, 20 games for streak calculation
  (
    SELECT COUNT(*)::text || '/5'
    FROM player_game_logs gl5
    WHERE gl5.player_id = p.player_id
      AND gl5.prop_type = p.prop_type
      AND gl5.sport = p.league
      AND gl5.date < p.date_normalized
      AND gl5.value >= p.line
    ORDER BY gl5.date DESC
    LIMIT 5
  ) as last5_streak,
  
  (
    SELECT COUNT(*)::text || '/10'
    FROM player_game_logs gl10
    WHERE gl10.player_id = p.player_id
      AND gl10.prop_type = p.prop_type
      AND gl10.sport = p.league
      AND gl10.date < p.date_normalized
      AND gl10.value >= p.line
    ORDER BY gl10.date DESC
    LIMIT 10
  ) as last10_streak,
  
  (
    SELECT COUNT(*)::text || '/20'
    FROM player_game_logs gl20
    WHERE gl20.player_id = p.player_id
      AND gl20.prop_type = p.prop_type
      AND gl20.sport = p.league
      AND gl20.date < p.date_normalized
      AND gl20.value >= p.line
    ORDER BY gl20.date DESC
    LIMIT 20
  ) as last20_streak,
  ...
  (
    ...
    FROM player_game_logs glh
    WHERE glh.player_id = p.player_id
      AND glh.prop_type = p.prop_type
      AND glh.sport = p.league
      AND glh.date < p.date_normalized
      AND glh.opponent = p.opponent
      AND glh.value >= p.line
  ) as h2h_streak,
