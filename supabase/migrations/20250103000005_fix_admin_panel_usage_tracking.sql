-- Fix admin panel usage tracking issues
-- This migration fixes RLS policies and ensures all required tables exist

-- First, let's check what the actual column name is in profiles table
-- and fix the RLS policies accordingly

-- Drop existing policies that might be using wrong column names
...
-- Create corrected RLS policies using the correct column name
-- Based on the profiles table structure, we'll use subscription_status
...
-- Create R2 usage tables if they don't exist
...
-- Create RLS policies for R2 tables
...
-- Insert default R2 plan configuration
...
INSERT INTO public.r2_current_usage (
  ...
  'statpedia-player-props-cache',
  1073741824, -- 1 GB
  50000,      -- 50K Class A operations
  500000,     -- 500K Class B operations
  536870912,  -- 512 MB egress
  0.15        -- $0.15 estimated cost
...
INSERT INTO public.api_current_usage (
  ...
  (SELECT user_id FROM public.profiles WHERE subscription_status = 'admin' LIMIT 1),
  1500,
  45000,
  1200,
  300,
  '{"player-props": 800, "events": 400, "odds": 300}'::jsonb,
  '{"nfl": 600, "nba": 500, "mlb": 400}'::jsonb,
  1.50
) ON CONFLICT (user_id, current_month_start) DO NOTHING;
