-- Teams table for canonical team name resolution
-- Replaces hardcoded team mappings with database-backed registry
...
-- Seed NFL data (expand later to MLB/NBA/NHL)
-- Minimal seed to eliminate UNK immediately. Add all teams as needed.
...
-- Spot missing abbreviations or logos
select league, team_name, abbreviation, logo_url
from teams
where abbreviation is null or logo_url is null;

-- Check aliases shape
select league, team_name, jsonb_array_length(aliases) as alias_count
from teams
order by alias_count desc, team_name;
