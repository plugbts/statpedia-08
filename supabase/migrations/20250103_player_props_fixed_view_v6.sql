...
-- 7. FIXED: Get team data from player_game_logs since proplines doesn't have team info
...
SELECT 
  ...
  -- Team abbreviations and logos: Use teams lookup table with team data from game logs
  COALESCE(t1.abbreviation, gl.team, p.team) as team_abbr,
  COALESCE(t2.abbreviation, gl.opponent, p.opponent) as opponent_abbr,
  ...
  COALESCE(
    (
      ...
      WHERE sh.player_id = p.player_id
        ...
        AND sh.opponent = COALESCE(gl.opponent, p.opponent)
        AND sh.value >= p.line
    ), '0/0'
  ) as h2h_streak,
...
FROM proplines p
-- Join to get player names and team data from game logs
...
-- Join to teams table for team abbreviations and logos (use team data from game logs)
LEFT JOIN teams t1 ON t1.team_name = COALESCE(gl.team, p.team) AND t1.league = p.league
LEFT JOIN teams t2 ON t2.team_name = COALESCE(gl.opponent, p.opponent) AND t2.league = p.league
