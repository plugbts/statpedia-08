...
class BackgroundPollerService {
  ...
  async fetchFromSportGameOdds(sport: string): Promise<any> {
    ...
    console.log(`Background polling SportGameOdds API for ${sport}: ${url.replace(config.sportsgameodds_api_key, 'REDACTED')}`);
    ...
    console.log(`SportGameOdds Rate Limit for ${sport} - Limit: ${rateLimit}, Remaining: ${rateLimitRemaining}, Reset: ${rateLimitReset}`);
  ...
  async updateCache(cacheKey: string, endpoint: string, sport: string, data: any, ttlSeconds: number): Promise<void> {
  ...
  async logPollingActivity(sport: string, success: boolean, responseTime: number, propsCount: number, error?: string): Promise<void> {
    await supabase
      .from('api_usage_logs')
      .insert({
    ...
    console.log(`Polling ${sport}: ${success ? 'SUCCESS' : 'FAILED'} - ${propsCount} props in ${responseTime}ms${error ? ` - ${error}` : ''}`);
  }
  ...
  async pollSport(sport: string): Promise<void> {
    ...
    try {
      ...
      const processedProps = await this.processPlayerProps(apiResponse, sport, config.max_props_per_request);
      propsCount = processedProps.length;

      // Update cache
      const cacheKey = `player-props-${sport}`;
      await this.updateCache(cacheKey, 'player-props', sport, processedProps, config.cache_ttl_seconds);
  ...
  getStatus() {
    return {
      ...
      config: this.config,
      nextPoll: this.pollInterval ? new Date(Date.now() + (this.config?.polling_interval_seconds || 30) * 1000) : null
    };
  }
}
