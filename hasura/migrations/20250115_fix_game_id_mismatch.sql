...
-- This creates a mapping table and resolves the ID mismatch
...
-- 2. Backfill the mapping table from games_canonical
-- (using external_id as the API game ID)
...
-- 3. Check what we have in the mapping table
SELECT COUNT(*) as mapped_games FROM game_id_map;

-- 4. Check the current mismatch count
SELECT COUNT(*) as orphaned_logs 
FROM player_game_logs pgl
LEFT JOIN games_canonical g ON g.id = pgl.game_id
WHERE g.id IS NULL;

-- 5. For now, let's create some sample games to match the orphaned logs
-- This is a temporary fix - in production you'd want to ingest real games first
...
SELECT 
    ...
    (SELECT id FROM teams_canonical WHERE league = pgl.league LIMIT 1) as home_team_id,
    (SELECT id FROM teams_canonical WHERE league = pgl.league OFFSET 1 LIMIT 1) as away_team_id,
    COALESCE(pgl.league, 'NFL') as league,
    CURRENT_DATE + INTERVAL '1 day' as game_date,
    2025 as season,
    1 as week
FROM (
    SELECT DISTINCT game_id, league 
    FROM player_game_logs pgl
    LEFT JOIN games_canonical g ON g.id = pgl.game_id
    WHERE g.id IS NULL
) pgl
...
-- 6. Update the mapping table with the new games
INSERT INTO game_id_map (api_game_id, canonical_game_id)
SELECT DISTINCT g.external_id, g.id
FROM games_canonical g
...
-- 7. Update player_game_logs to use canonical IDs
...
-- 9. Enforce foreign key constraint going forward
ALTER TABLE player_game_logs
  DROP CONSTRAINT IF EXISTS fk_player_game_logs_game;

ALTER TABLE player_game_logs
  ADD CONSTRAINT fk_player_game_logs_game
  FOREIGN KEY (game_id) REFERENCES games_canonical(id);

-- 10. Test the join works now
