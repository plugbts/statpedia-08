...
SELECT 
    pp.id AS prop_id,
    g.league,
    g.league AS sport, -- Added for backward compatibility
    pp.game_id,
    g.game_date,
    pp.market,
    pp.line,
...
JOIN teams_canonical home_t ON home_t.id = g.home_team_id
JOIN teams_canonical away_t ON away_t.id = g.away_team_id
