-- Comprehensive enrichment function that computes real rolling averages and streaks
...
BEGIN
    ...
    WITH rolling_stats AS (
        ...
        SELECT DISTINCT
            ...
            -- Rolling averages based on actual_value
            AVG(pgl.actual_value) OVER (PARTITION BY pgl.player_id ORDER BY pgl.game_date ROWS BETWEEN 4 PRECEDING AND CURRENT ROW) AS l5,
            AVG(pgl.actual_value) OVER (PARTITION BY pgl.player_id ORDER BY pgl.game_date ROWS BETWEEN 9 PRECEDING AND CURRENT ROW) AS l10,
            AVG(pgl.actual_value) OVER (PARTITION BY pgl.player_id ORDER BY pgl.game_date ROWS BETWEEN 19 PRECEDING AND CURRENT ROW) AS l20,
    ...
    streak_calc AS (
        -- Compute streaks
        WITH ordered AS (
            SELECT player_id, game_date, actual_value,
                   CASE WHEN actual_value >= 5 THEN 1 ELSE 0 END AS hit
    ...
    SELECT DISTINCT
        ...
        CONCAT(
            ...
            (SELECT COUNT(*) FROM player_game_logs pgl2 WHERE pgl2.player_id = rs.player_id)::TEXT
        ) AS streak,
