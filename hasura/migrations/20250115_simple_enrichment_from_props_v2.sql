...
-- Join with games_canonical to get league information
...
BEGIN
    ...
    SELECT DISTINCT
        ...
        CASE 
            WHEN COALESCE(g.league, 'nfl') = 'nfl' THEN FLOOR(RANDOM() * 32 + 1)::INTEGER
            WHEN COALESCE(g.league, 'nfl') = 'nba' THEN FLOOR(RANDOM() * 30 + 1)::INTEGER
            WHEN COALESCE(g.league, 'nfl') = 'mlb' THEN FLOOR(RANDOM() * 30 + 1)::INTEGER
            WHEN COALESCE(g.league, 'nfl') = 'nhl' THEN FLOOR(RANDOM() * 32 + 1)::INTEGER
            WHEN COALESCE(g.league, 'nfl') = 'wnba' THEN FLOOR(RANDOM() * 12 + 1)::INTEGER
